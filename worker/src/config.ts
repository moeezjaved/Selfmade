...
export const config = {
  ...
  // 4GB RAM → 5-6 concurrent. 8GB → 10-12. 16GB → 15-20.
  ...
  // Sleep between batches when busy (ms)
  batchSleep: num('WORKER_BATCH_SLEEP_MS', 500),
  // Per-ad timeout (extracts CDN URL + download + upload)
  adTimeoutMs: num('WORKER_AD_TIMEOUT_MS', 60_000),
