...
- hook_type: first-3-seconds pattern. beats: ONE entry PER DISTINCT CUT/SHOT in the ad — every hard cut to a new location, subject, angle or camera setup is its own beat, in order, each with its exact time range. Match beats to scene_count: a fast-cut ad has many (typically 8-20, up to 24), a single continuous take has 1. Do NOT summarise several cuts into one beat — a clone reproduces the ad shot by shot from these, so a missing beat is a missing scene. transcript: exact spoken words. Be concrete.
...
${(beat && beat.rejected_product && String(beat.rejected_product).trim()) ? `- REJECTED PRODUCT — the ad contrasts a BAD/rival thing ("${String(beat.rejected_product).replace(/"/g, "'").slice(0, 80)}") against the hero. In any beat that shows that rejected thing, KEEP IT AS-IS (${String(beat.rejected_product).replace(/"/g, "'").slice(0, 80)}) — do NOT swap it to the user's product. Only the HERO product becomes the user's. NEVER turn the rejected thing into the user's product (that breaks the story: the bad thing must stay the bad thing). Set "has_product": false on a beat that shows ONLY the rejected thing.` : ''}
- COUNT FIDELITY: show the user's product the SAME NUMBER OF TIMES it truly appears in that beat — usually ONE unit in hand. Do NOT multiply it (two devices when the source held one, or the source held two of the REJECTED thing) — match the real count, and default to a single unit unless the reference clearly shows more of the HERO product.
${productTruthRule(product, isService)}
...
- CAST: label each distinct person "A", "B", "C", "D", "E"… (A = the main character) and use the SAME letter for the same person across every scene. Per scene report "cast": ALL the letters visible in that scene ([] when no people). Use as many letters as there are genuinely distinct people in the ad — don't collapse an ensemble into one person, and don't invent people who aren't there.
...
- CAST PROFILES: also return "cast_profiles" — an object mapping each cast letter used ("A","B"…) to a DENSE look description of that exact person (age, gender, hair, build, wardrobe, distinguishing features), ${recast ? `with ETHNICITY/skin tone RECAST as ${look} for EVERY person (the recast overrides the reference's ethnicity — keep each person's gender, age, build, hairstyle and wardrobe, but make them ${look}; never carry over the reference's original ethnicity like "Black"/"white"/"Asian")` : 'copied faithfully from the reference including ethnicity'}. Use the reference "people" list${beat && Array.isArray(beat.people) && beat.people.length ? ` = ${JSON.stringify(beat.people).slice(0, 400)}` : ''} when present. This lets us draw a locked character sheet for EACH person, not just the lead.
Return ONLY minified JSON: {"scenes":[{"prompt":"","script":"","duration":5,"has_people":false,"has_product":true,"cast":["A"],"setting":"","lens":"","src_start":0,"src_end":5}],"cast_profiles":{"A":""}}  (exactly ${nScenes} scenes, in order).`
  ...
  const mapped = picked.map((s) => {
    ...
    return {
      ...
      cast: Array.isArray(s.cast) ? s.cast.map(String).slice(0, 4) : (s.has_people !== false ? ['A'] : []),
    ...
    if (meta.mode === 'faithful') {
      ...
      try {
        ...
        for (let i = 0; i < scenes.length; i++) {
          ...
          const anchorRefs = [...new Set(cast.map((c) => charAnchors[c]).filter(Boolean))].slice(0, 4)
