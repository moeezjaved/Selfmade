...
Return ONLY JSON:
{
  ...
  "leaks": [ {"title","why (START by quoting or naming the EXACT on-page element/text this is about, e.g. \\"The line 'A powerful solution for who want...' has a grammar error\\", so the founder can find it — never a vague label)","fix","screen":"home" or "pdp","severity":"critical|high|medium"${grounded ? ',"evidence":"ev_x"' : ''}${grounded ? ',"region":{"x":0-100,"y":0-100,"w":0-100,"h":0-100} — REQUIRED for every leak visible on a screenshot: your best-estimate box (as % of that page\'s screenshot) around the exact element. Approximate is fine and expected (the UI labels pins "approximate") — do NOT omit it; only leave it out for a leak that is genuinely not visible in the screenshot' : ''}} ],  // EXACTLY 5, most severe first
