  ...
  return [
    ...
    opts.isService
      ...
      : opts.numProducts > 0
        ...
        // No product photo attached — build a believable branded product ad from the product DESCRIPTION;
        // depict a plausible on-brand product/scene, never leave the frame empty or show another brand's item.
        : `NO product photo is attached. Depict the product plausibly from its description below and make it the hero, or lead with a real person clearly enjoying the result — bold editorial typography and brand color throughout. Never show another brand's product.`,
    opts.isService || opts.numProducts === 0 ? '' : `Render the product 1:1 from the photo(s): match its EXACT silhouette, proportions, materials, textures, and on-label branding/text. Do NOT reshape, restyle, or invent a different product.`,
    opts.isService || opts.numProducts === 0 ? '' : `SIZE THE PRODUCT REALISTICALLY — this is critical. Render it at its true real-world size RELATIVE TO the scene: a small handheld device must look small in a hand, in correct proportion to fingers, faces, furniture, and surroundings. Keep it to roughly a QUARTER of the frame; never enlarge it, never make it larger-than-life, never let it dominate the composition. If a person holds it, it must look natural in their grip, not oversized. Leave clear negative space around it. It is the only product shown.`,
