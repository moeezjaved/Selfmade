...
export function buildClonePrompt(opts: {
  ...
  return [
    ...
    `ADAPT THE SETTING to the user's product: if image 1's background, surface, spilled ingredients, or props are SPECIFIC to the original product's category (e.g. coffee beans behind a coffee bag, pills behind a supplement), replace them with a background/ingredients/props that make sense for the USER'S product (read its photo to infer what fits — e.g. tomatoes, herbs, olive oil, a kitchen counter for a pesto). Keep the same composition, framing, lighting, and premium mood — but the scene must suit the user's product, not the original's. Generic/neutral backgrounds can stay as-is.`,
