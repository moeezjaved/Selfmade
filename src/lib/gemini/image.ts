...
export function buildStudioPrompt(opts: {
  ...
  return [
    ...
    opts.brief ? `FOUNDER'S EXACT REQUEST — honour every specific detail they named (the people/cast and their ethnicity/look, the setting, era, mood and props): "${opts.brief}". These specifics are REQUIRED — do not generalise them away. This applies to the SCENE only; the product and logo still stay exactly as specified above (never restyled to match the request).` : '',
