...
(function(){
 try {
  ...
  // NOTE: EDITOR_JS is a template literal — backslash escapes (\\d, \\s, \\() are eaten when it's cooked,
  // silently breaking any regex that uses them (and killing the whole runtime). Keep regexes backslash-free.
  function rgbToHex(c){ var m=/rgba?[(]([0-9]+)[, ]+([0-9]+)[, ]+([0-9]+)/.exec(c||''); if(!m) return '#000000'; return '#'+[1,2,3].map(function(i){ return ('0'+parseInt(m[i],10).toString(16)).slice(-2); }).join(''); }
  ...
  function insertBlock(html, anchorId, position){
    ...
    buildSections(); openAllDetails(); markDirty();
