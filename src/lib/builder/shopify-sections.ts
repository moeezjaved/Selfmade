...
const productForm = (label: string, cls: string, withOptions = true): string => {
  ...
  const pickers = withOptions
    ? `{% unless product.has_only_default_variant %}<div class="sf-variants" style="margin:0 0 14px">{% for opt in product.options_with_values %}{% assign sfcolor = false %}{% if opt.name contains 'olor' or opt.name contains 'olour' %}{% assign sfcolor = true %}{% endif %}<div class="sf-optrow" data-opt="{{ forloop.index0 }}"><div class="sf-optname">{{ opt.name }}: <b class="sf-optval">{{ opt.selected_value }}</b></div><div class="sf-optvals">{% for val in opt.values %}<button type="button" class="sf-pill{% if sfcolor %} sf-color{% endif %}{% if opt.selected_value == val %} on{% endif %}" data-value="{{ val | escape }}">{% if sfcolor %}<span class="sf-dot" style="background:{{ val | downcase | replace: ' ','' | replace: '/','' }}"></span>{% endif %}{{ val }}</button>{% endfor %}</div></div>{% endfor %}</div><script type="application/json" data-sf-vdata>{{ product.variants | json }}</script>{% endunless %}`
    : ''
