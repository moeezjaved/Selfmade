...
const THUMBS_SCRIPT = `<script>(function(){var s=document.currentScript;var root=s?s.parentElement:document;var scope=root.closest('.pgbld')||root;if(scope.querySelector('.gtrack'))return;var thumbs=[].slice.call(scope.querySelectorAll('.thumbs img'));if(thumbs.length<2)return;var main=scope.querySelector('.hbottle')||scope.querySelector('.gimg')||scope.querySelector('.gallery img')||scope.querySelector('.hero img:not(.thumbs img)');if(!main||main.closest('.thumbs'))return;function mark(i){thumbs.forEach(function(x,j){x.classList.toggle('on',j===i);});}var ci=0;thumbs.forEach(function(t,i){t.style.cursor='pointer';t.addEventListener('click',function(){var src=t.getAttribute('src');if(!src)return;ci=i;main.setAttribute('src',src);mark(i);});});var srcs=thumbs.map(function(t){return t.getAttribute('src')||'';});function go(d){var n=srcs.length;ci=((ci+d)%n+n)%n;main.setAttribute('src',srcs[ci]);mark(ci);}var pv=scope.querySelector('.gprev'),nx=scope.querySelector('.gnext');if(pv)pv.addEventListener('click',function(){go(-1);});if(nx)nx.addEventListener('click',function(){go(1);});})();</script>`
...
const LIST_CONTAINERS: Array<[string, string]> = [
  ...
  ['ppills', 'Pill'], ['sgrid', 'Stat'], ['press', 'Quote'], ['fgrid', 'Feature'], ['hchecks', 'Benefit'],
]
...
export function buildThemeAssets(opts: { pageId: string; kind: PageKind; css: string; body: string; templateSuffix: string; dynamic?: DynamicMode }): ThemeAssets {
  ...
  // Keep a safe margin under Shopify's 25-section ceiling (some themes/templates reject at the boundary):
  // aim for ≤20 total (product reserves a slot for the recommendations section).
  const CAP = opts.kind === 'product' && dyn !== 'none' ? 19 : 20
