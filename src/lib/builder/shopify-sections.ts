...
const THUMBS_SCRIPT = `<script>(function(){var s=document.currentScript;var root=s?s.parentElement:document;var scope=root.closest('.pgbld')||root;if(scope.querySelector('.gtrack'))return;var thumbs=[].slice.call(scope.querySelectorAll('.thumbs img'));if(thumbs.length<2)return;var main=scope.querySelector('.hbottle')||scope.querySelector('.gimg')||scope.querySelector('.gallery img')||scope.querySelector('.hero img:not(.thumbs img)');if(!main||main.closest('.thumbs'))return;function mark(i){thumbs.forEach(function(x,j){x.classList.toggle('on',j===i);});}thumbs.forEach(function(t,i){t.style.cursor='pointer';t.addEventListener('click',function(){var src=t.getAttribute('src');if(!src)return;main.setAttribute('src',src);mark(i);});});var srcs=thumbs.map(function(t){return t.getAttribute('src')||'';});function cur(){return srcs.indexOf(main.getAttribute('src')||'');}function go(d){var k=cur();var n=srcs.length;var i=((k<0?(d>0?-1:0):k)+d%n+n)%n;main.setAttribute('src',srcs[i]);mark(i);}var pv=scope.querySelector('.gprev'),nx=scope.querySelector('.gnext');if(pv)pv.addEventListener('click',function(){go(-1);});if(nx)nx.addEventListener('click',function(){go(1);});})();</script>`
