...
export function emailShell(opts: { title: string; intro: string; ctaText?: string; ctaUrl?: string; confirmUrl?: string; imageUrl?: string }): string {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="padding:28px 12px;"><tr><td align="center">
    <table role="presentation" width="100%" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0;">
      <tr><td style="background:#ef4a1e;padding:20px 28px;"><span style="color:#ffffff;font-weight:800;font-size:18px;letter-spacing:-.02em;">Selfmade</span></td></tr>
      ...
      <tr><td style="padding:28px;">
        ...
        ${ctaText && ctaUrl ? `<a href="${ctaUrl}" style="display:inline-block;background:#ef4a1e;color:#ffffff;text-decoration:none;font-weight:700;font-size:15px;padding:12px 22px;border-radius:10px;">${ctaText}</a>` : ''}
        ${confirmUrl ? `<div style="margin-top:22px;padding:14px 16px;background:#fef1ec;border:1px solid #f6d3c4;border-radius:12px;">
          <p style="margin:0 0 10px;color:#7a2d15;font-size:13px;line-height:1.5;"><b>One quick thing:</b> confirm your email so we can send you fresh winning-ad picks. It keeps us out of your spam folder too.</p>
          <a href="${confirmUrl}" style="display:inline-block;background:#ef4a1e;color:#ffffff;text-decoration:none;font-weight:700;font-size:13px;padding:9px 16px;border-radius:8px;">Confirm my email</a>
        </div>` : ''}
      </td></tr>
