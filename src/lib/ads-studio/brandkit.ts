...
Write "facts": 26-32 RICH, self-contained observations — aim for the higher end when the content supports it. Follow these rules exactly — they are what separates an excellent knowledge base from a mediocre one:
...
- Cover as many of these dimensions as the content supports, and give the meaty ones their OWN separate facts rather than compressing them: positioning; the core product with its materials and build; what it explicitly is NOT / how it differs from alternatives (make differentiation its own distinct fact listing the concrete points); the mechanism (how it works); how a customer actually uses it step by step; the specific product variants with their distinguishing details; bundles/packs/quantities (cite the FAQ when it gives numbers or durations); subscription or recurring offers; the e-commerce platform and standard flows (Shopify, "Shop All", cart, checkout, etc.); pricing angles and value claims; guarantees, shipping and returns; the primary use cases and the problem being solved; the FUNCTIONAL pain points it addresses AND the PSYCHOLOGICAL/emotional pain points (guilt, social stigma, feeling out of control) as separate facts; the outcomes it promises, noting whether they are behavioral/emotional vs medical; the target audience; proof and marketing claims (report claims AS claims, e.g. "the site claims…"); brand voice and mindset; and any geography or market signals.
- Every fact must be traceable to the content above. If a detail isn't in the content, omit it — do not guess.

Also write "voice": {"tone": one-or-two-word brand tone (e.g. "Mindful premium"), "energy": "Low"|"Medium"|"High", "audience": a specific phrase naming the core audience AND what they want (e.g. "Health-conscious young adults seeking a nicotine-free alternative"), not just a demographic}.
