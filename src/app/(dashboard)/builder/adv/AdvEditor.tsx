...
// Green trust palette (money-back / guarantee badges use green universally, matching PagePilot).
const SL_GREEN = '#0f7a44', SL_GREEN_BG = '#eafaf1', SL_GREEN_LINE = '#bfe6cf', SL_GREEN_DK = '#123d26'
// Polished, dependency-free placeholder art (data-URI SVG) so gallery previews look real, not "92x9" grey boxes.
// Still plain <img> so the merchant can swap them in the Image panel.
const svgURI = (svg: string) => 'data:image/svg+xml,' + encodeURIComponent(svg)
const SL_AVATAR = svgURI(`<svg xmlns="http://www.w3.org/2000/svg" width="96" height="96"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#cbd0f8"/><stop offset="1" stop-color="#8f99ee"/></linearGradient></defs><rect width="96" height="96" fill="url(#g)"/><circle cx="48" cy="37" r="17" fill="#fff" opacity=".92"/><path d="M17 90c0-18 15-27 31-27s31 9 31 27z" fill="#fff" opacity=".92"/></svg>`)
const SL_VIDEO = svgURI(`<svg xmlns="http://www.w3.org/2000/svg" width="360" height="520"><defs><linearGradient id="v" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#e2e6fb"/><stop offset="1" stop-color="#b4bdf3"/></linearGradient></defs><rect width="360" height="520" fill="url(#v)"/><circle cx="180" cy="200" r="58" fill="#fff" opacity=".55"/><path d="M70 520c0-72 50-116 110-116s110 44 110 116z" fill="#fff" opacity=".55"/></svg>`)
// Distinct-toned avatar placeholders so an avatar row reads as different people (merchant swaps for real photos).
const slAvatar = (a: string, b: string) => svgURI(`<svg xmlns="http://www.w3.org/2000/svg" width="96" height="96"><defs><linearGradient id="a" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${a}"/><stop offset="1" stop-color="${b}"/></linearGradient></defs><rect width="96" height="96" fill="url(#a)"/><circle cx="48" cy="37" r="17" fill="#fff" opacity=".92"/><path d="M17 90c0-18 15-27 31-27s31 9 31 27z" fill="#fff" opacity=".92"/></svg>`)
const SL_AV2 = slAvatar('#f6c7b6', '#e79a7a'), SL_AV3 = slAvatar('#bfe3c9', '#7fc79a')
// Blue "verified buyer" badge (Twitter-style) shown before reviewer names, like PagePilot.
const SL_VBADGE = `<span style="display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;border-radius:50%;background:#1d9bf0;color:#fff;font-size:11px;font-weight:900;flex:none">&#10003;</span>`
const SL_PHOTO = svgURI(`<svg xmlns="http://www.w3.org/2000/svg" width="720" height="520"><defs><linearGradient id="p" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#eef0fe"/><stop offset="1" stop-color="#cdd4f6"/></linearGradient></defs><rect width="720" height="520" fill="url(#p)"/><circle cx="360" cy="215" r="66" fill="#fff" opacity=".55"/><path d="M150 520c0-96 94-150 210-150s210 54 210 150z" fill="#fff" opacity=".55"/></svg>`)
const slIcon = (paths: string, stroke = SL_ACCENT, size = 28) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="${stroke}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${paths}</svg>`
const SL_SHIELD = (c: string) => slIcon('<path d="M12 3l7 3v6c0 4.6-3.1 7.6-7 9-3.9-1.4-7-4.4-7-9V6z"/><path d="M9 12l2 2 4-4"/>', c, 30)
...
// Real-looking payment brand marks (like PagePilot), each on a white card. Recognisable brand colours, no external assets.
const slPayCard = (inner: string, bg = '#fff') => `<span style="display:inline-flex;align-items:center;justify-content:center;width:46px;height:30px;background:${bg};border:1px solid #e7e3dd;border-radius:6px;box-shadow:0 1px 2px rgba(0,0,0,.06)">${inner}</span>`
const slPays = () => `<div class="pays" style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;align-items:center;margin-top:18px">${[
  slPayCard(`<span style="color:#1a1f71;font-style:italic;font-weight:900;font-size:13px;letter-spacing:-.3px">VISA</span>`),
  slPayCard(`<span style="position:relative;display:inline-block;width:32px;height:19px"><span style="position:absolute;left:2px;top:0;width:19px;height:19px;border-radius:50%;background:#eb001b"></span><span style="position:absolute;left:11px;top:0;width:19px;height:19px;border-radius:50%;background:#f79e1b;mix-blend-mode:multiply"></span></span>`),
  slPayCard(`<span style="color:#fff;font-weight:800;font-size:9px;letter-spacing:.3px">AMEX</span>`, '#1f72cd'),
  slPayCard(`<span style="font-weight:900;font-size:11px;font-style:italic"><span style="color:#003087">Pay</span><span style="color:#009cde">Pal</span></span>`),
  slPayCard(`<span style="display:inline-flex;align-items:center;gap:2px;font-weight:600;font-size:11px;color:#fff"><svg width="10" height="12" viewBox="0 0 14 17" fill="#fff"><path d="M11.2 9c0-1.9 1.5-2.8 1.6-2.9-.9-1.3-2.3-1.5-2.7-1.5-1.2-.1-2.3.7-2.9.7-.6 0-1.5-.7-2.5-.7-1.3 0-2.5.7-3.1 1.9-1.3 2.3-.3 5.7 1 7.6.6.9 1.3 1.9 2.3 1.9.9 0 1.3-.6 2.4-.6s1.4.6 2.4.6 1.6-.9 2.2-1.8c.7-1 1-2 1-2.1-.1 0-1.9-.7-1.9-2.6zM9.4 3.4c.5-.6.9-1.5.8-2.4-.8 0-1.7.5-2.3 1.2-.5.6-.9 1.5-.8 2.3.9.1 1.7-.5 2.3-1.1z"/></svg>Pay</span>`, '#000'),
  slPayCard(`<span style="display:inline-flex;align-items:center;font-weight:600;font-size:11px"><span style="color:#4285F4">G</span><span style="color:#5f6368">&nbsp;Pay</span></span>`),
  slPayCard(`<span style="color:#fff;font-weight:900;font-size:12px">shop</span>`, '#5a31f4'),
].join('')}</div>`
...
const SECTION_LIBRARY: { id: string; label: string; html: string }[] = [
  ...
  { id: 'reviews', label: 'Featured Reviews Carousel', html: slWrap(`<div class="frcar" style="position:relative;max-width:800px;margin:0 auto;padding:0 40px"><div class="frtrack" style="display:flex;gap:16px;overflow-x:auto;scroll-snap-type:x mandatory;scrollbar-width:none">${([[SL_AVATAR, 'John Doe', 'Using this changed my business forever. It feels like I\'m working at super speed when creating product pages and it literally doubled my conversion rate! It\'s a must-have tool for every e-commerce entrepreneur!'], [SL_AV2, 'Sarah M.', 'Honestly the best I have tried — I noticed a difference within days. It has become part of my weekly routine and I recommend it to everyone.'], [SL_AV3, 'James T.', 'Worth every penny. Fast, effective, and it actually works. Repurchasing for the third time now — highly recommend.']] as [string, string, string][]).map(([av, n, q]) => `<div class="frcard" style="scroll-snap-align:center;flex:0 0 100%;box-sizing:border-box;background:${SL_WASH};border-radius:18px;padding:26px 30px;display:flex;align-items:center;gap:26px"><span class="avim" style="width:96px;height:96px;border-radius:50%;overflow:hidden;flex:none"><img src="${av}" alt="" style="width:100%;height:100%;object-fit:cover"></span><div><div class="stars" style="color:#f5c518;letter-spacing:2px;font-size:19px;margin-bottom:10px">★★★★★</div><p class="frq" style="font-size:15.5px;line-height:1.6;color:${SL_INK};margin:0 0 12px">"${q}"</p><div class="frwho" style="display:flex;align-items:center;gap:8px">${SL_VBADGE}<span class="frname" style="font-size:14.5px;font-weight:800;color:${SL_INK}">${n}</span></div></div></div>`).join('')}</div><span class="frprev" style="position:absolute;left:0;top:50%;transform:translateY(-50%);width:40px;height:40px;border-radius:50%;background:#fff;border:1px solid ${SL_LINE};box-shadow:0 2px 10px rgba(20,18,15,.18);display:flex;align-items:center;justify-content:center;color:${SL_INK};font-size:20px;line-height:1;cursor:pointer">‹</span><span class="frnext" style="position:absolute;right:0;top:50%;transform:translateY(-50%);width:40px;height:40px;border-radius:50%;background:#fff;border:1px solid ${SL_LINE};box-shadow:0 2px 10px rgba(20,18,15,.18);display:flex;align-items:center;justify-content:center;color:${SL_INK};font-size:20px;line-height:1;cursor:pointer">›</span></div>`) },
  { id: 'as-seen-on', label: 'As Seen On', html: slWrap(`<div style="text-align:center;font-size:15px;font-weight:800;color:${SL_INK};margin-bottom:24px">As Seen On</div><div class="press" style="display:flex;flex-wrap:wrap;gap:46px;justify-content:center;align-items:center">
      <span class="plogo" style="font-family:Arial,Helvetica,sans-serif;font-size:23px;font-weight:700;color:#b5b5b5;letter-spacing:-.5px">Bloomberg</span>
      <span class="plogo" style="font-family:Georgia,'Times New Roman',serif;font-size:20px;font-weight:800;color:#111;letter-spacing:1px">COSMOPOLITAN</span>
      <span class="plogo" style="font-family:Georgia,'Times New Roman',serif;font-size:21px;font-weight:800;color:#111;letter-spacing:-.3px">Women'sHealth</span>
      <span class="plogo" style="font-family:Georgia,'Times New Roman',serif;font-size:26px;font-weight:400;color:#c2c2c2;letter-spacing:1px">allure</span>
    </div>`) },
  // ── Social Proof & Trust — PagePilot-parity blocks (each drops in as its own editable section) ──
  { id: 'trusted-customers', label: 'Trusted by thousands customers', html: slWrap(`<div style="text-align:center"><div class="tbc" style="display:inline-flex;align-items:center;gap:16px;background:#f1f2f4;border-radius:16px;padding:14px 22px;text-align:left;max-width:100%"><div class="avs" style="display:flex;position:relative;flex:none">${[SL_AVATAR, SL_AV2, SL_AV3].map((av, i) => `<span class="avim" style="width:44px;height:44px;border-radius:50%;border:2px solid #fff;margin-left:${i ? -14 : 0}px;overflow:hidden"><img src="${av}" alt="" style="width:100%;height:100%;object-fit:cover"></span>`).join('')}<span style="position:absolute;right:-5px;bottom:-3px;width:19px;height:19px;border-radius:50%;background:#1d9bf0;border:2px solid #fff;display:flex;align-items:center;justify-content:center;color:#fff;font-size:11px;font-weight:900">&#10003;</span></div><div><div class="tbt" style="font-size:17px;font-weight:800;color:${SL_INK}">Trusted by thousands customers</div><div class="tbs" style="font-size:14px;color:${SL_SUB};font-style:italic">They choose us for premium quality and a satisfaction guarantee.</div></div></div></div>`) },
  { id: 'video-testimonials', label: 'Video Testimonials Carousel', html: slWrap(slHead('They tried it. They adopted it.') + `<div class="vcar" style="display:flex;gap:14px;overflow-x:auto;padding:4px 2px;scrollbar-width:none">${[1, 2, 3, 4].map(() => `<div class="vcard" style="flex:0 0 210px;max-width:210px;position:relative;border-radius:16px;overflow:hidden;aspect-ratio:9/16;background:#000"><img src="${SL_VIDEO}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"><span style="position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,0) 55%,rgba(0,0,0,.45) 100%)"></span><span style="position:absolute;left:50%;top:44%;transform:translate(-50%,-50%);width:52px;height:52px;border-radius:50%;background:rgba(255,255,255,.92);display:flex;align-items:center;justify-content:center;font-size:19px;color:#111;padding-left:4px">▶</span></div>`).join('')}</div>`) },
  { id: 'reviews-number', label: 'Reviews Number', html: slWrap(`<div class="rnum" style="display:flex;align-items:center;justify-content:center;gap:12px;flex-wrap:wrap"><span class="stars" style="color:#f5c518;letter-spacing:2px;font-size:24px">★★★★★</span><span class="rtext" style="font-size:17px;color:${SL_INK}">Rated 4.9/5 by 2340 people</span></div>`) },
  { id: 'featured-review', label: 'Featured Review', html: slWrap(`<div class="frev" style="display:flex;align-items:center;gap:30px;max-width:760px;margin:0 auto"><span class="avim" style="width:104px;height:104px;border-radius:50%;overflow:hidden;flex:none"><img src="${SL_AVATAR}" alt="" style="width:100%;height:100%;object-fit:cover"></span><div><div class="stars" style="color:#f5c518;letter-spacing:2px;font-size:20px;margin-bottom:12px">★★★★★</div><p class="frq" style="font-size:16px;line-height:1.65;color:${SL_INK};margin:0 0 14px">"Using this changed my business forever. It feels like I'm working at super speed when creating product pages and it literally doubled my conversion rate! It's a must-have tool for every e-commerce entrepreneur!"</p><div class="frwho" style="display:flex;align-items:center;gap:8px">${SL_VBADGE}<span class="frname" style="font-size:15px;font-weight:800;color:${SL_INK}">John Doe</span></div></div></div>`) },
  { id: 'review-card', label: 'Review Card', html: slWrap(`<div class="rcard" style="max-width:380px;margin:0 auto;background:#fff;border:1px solid ${SL_LINE};border-radius:18px;overflow:hidden;box-shadow:0 4px 18px -10px rgba(20,18,15,.2)"><img src="${SL_PHOTO}" alt="" style="width:100%;height:300px;object-fit:cover;display:block"><div style="padding:22px"><div class="rcwho" style="display:flex;align-items:center;gap:8px;margin-bottom:12px">${SL_VBADGE}<span class="rcname" style="font-size:15px;font-weight:800;color:${SL_INK}">John Doe</span></div><div class="stars" style="color:#f5c518;letter-spacing:2px;font-size:18px;margin-bottom:12px">★★★★★</div><p class="rcq" style="font-size:15px;line-height:1.6;color:${SL_INK};margin:0">"Built for modern eCommerce — fast, automated, and conversion focused. We simplify your workflow so you can test more products, launch quicker, and grow with confidence."</p></div></div>`) },
  // ── Trust & Credibility — PagePilot-parity blocks ──
  { id: 'guarantee-90', label: '90-Day Money Back Guarantee', html: slWrap(`<div style="text-align:center"><div class="g90" style="display:inline-flex;align-items:center;gap:16px;background:${SL_GREEN_BG};border:1px solid ${SL_GREEN_LINE};border-radius:16px;padding:18px 28px;text-align:left"><span class="pico" style="display:inline-flex;flex:none">${SL_SHIELD(SL_GREEN)}</span><div><div class="g90t" style="font-size:19px;font-weight:800;color:${SL_GREEN}">90-Day Money Back Guarantee</div><div class="g90s" style="font-size:15px;color:${SL_GREEN}">Full refund, no questions asked</div></div></div></div>`) },
  { id: 'money-back', label: 'Money Back Guarantee', html: slWrap(`<div class="mbg" style="display:flex;align-items:flex-start;gap:18px;max-width:680px;margin:0 auto;background:${SL_GREEN_BG};border:1.5px dashed ${SL_GREEN};border-radius:16px;padding:22px 26px"><span class="pico" style="width:54px;height:54px;border-radius:50%;background:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:26px;flex:none">💖</span><div><div class="mbt" style="font-size:16px;font-weight:800;color:${SL_GREEN};display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span style="background:${SL_GREEN_DK};color:#fff;font-size:11px;font-weight:800;padding:3px 9px;border-radius:5px">NO RISK</span>100% Money Back Guarantee</div><p class="mbs" style="font-size:14px;color:${SL_GREEN};line-height:1.6;margin:8px 0 0">We get it — you've been disappointed before. So here's our promise: try it for 30 days and if you're not feeling more like yourself again, we'll refund every penny. No hoops. No hassle.</p></div></div>`) },
  { id: 'brand-trust', label: 'Brand Trust', html: slWrap(`<div class="btrust" style="display:flex;flex-wrap:wrap;gap:52px;justify-content:center;align-items:center">${[['🇺🇸', 'Made in the USA'], ['🏆', 'Award Winning Product']].map(([i, t]) => `<div class="bti" style="display:flex;align-items:center;gap:12px;font-size:19px;font-weight:700;color:${SL_GREEN}"><span style="font-size:26px">${i}</span>${t}</div>`).join('')}</div>`) },
  { id: 'trustpilot', label: 'Trustpilot Rating', html: slWrap(`<div class="tpilot" style="display:flex;align-items:center;justify-content:center;gap:14px;flex-wrap:wrap"><span style="font-size:26px;font-weight:800;color:${SL_GREEN_DK}">Excellent</span><span style="display:inline-flex;gap:3px">${[0, 0, 0, 0, 0].map(() => `<span style="width:30px;height:30px;background:#00b67a;display:inline-flex;align-items:center;justify-content:center;color:#fff;font-size:18px;border-radius:2px">★</span>`).join('')}</span><span style="font-size:23px;font-weight:800;color:#191919;display:inline-flex;align-items:center;gap:5px"><span style="color:#00b67a;font-size:25px">★</span>Trustpilot</span></div>`) },
  { id: 'rated-by', label: 'Rated by', html: slWrap(`<div style="text-align:center"><div class="ratedby" style="display:inline-flex;align-items:center;gap:12px;background:#fdf5e0;border-radius:999px;padding:12px 24px"><span class="stars" style="color:#f5c518;letter-spacing:2px;font-size:20px">★★★★★</span><span class="rbt" style="font-size:15px;color:${SL_INK}">Rated <b>4.9/5</b> based on <b>2453+</b> Trusted Reviews</span></div></div>`) },
  { id: 'bestseller', label: 'Best Seller Badge', html: slWrap(`<div style="text-align:center"><div class="bsb" style="display:inline-flex;align-items:stretch;background:#fff;border:1px solid ${SL_LINE};border-radius:12px;overflow:hidden;text-align:left;box-shadow:0 3px 14px -8px rgba(20,18,15,.2)"><span style="background:${SL_ACCENT};color:#fff;font-weight:900;font-size:20px;display:flex;align-items:center;padding:0 18px;flex:none">#1</span><div style="padding:12px 22px"><div class="bsbt" style="font-size:15px;font-weight:800;color:${SL_INK};letter-spacing:.04em">BESTSELLER OF 2026</div><div class="bsbs" style="font-size:14px;color:${SL_SUB}">Trusted by Doctors Worldwide</div></div></div></div>`) },
  { id: 'payment-icons', label: 'Payment Icons', html: slWrap(`<div style="text-align:center">${slPays()}</div>`) },
  // ── Benefits & Features — PagePilot-parity blocks ──
  { id: 'spotlight-benefit', label: 'Spotlight Benefit', html: slWrap(`<div class="spot" style="display:flex;align-items:center;gap:18px;max-width:640px;margin:0 auto;background:${SL_WASH};border-radius:16px;padding:22px 26px"><span class="pico" style="display:inline-flex;flex:none">${slIcon('<path d="M12 2l2.4 7.4H22l-6 4.6 2.3 7.4L12 17l-6.3 4.4L8 14 2 9.4h7.6z"/>', SL_ACCENT, 30)}</span><div><div class="spott" style="font-size:16px;font-weight:800;color:${SL_INK}">Made for everyday use</div><p class="spots" style="font-size:14px;color:${SL_SUB};line-height:1.6;margin:6px 0 0">Thoughtfully designed to fit seamlessly into your routine — compact, practical, and easy to use wherever you are.</p></div></div>`) },
  { id: 'text-list-icons', label: 'Text List with Icons', html: slWrap(`<div class="tli" style="max-width:520px;margin:0 auto;display:flex;flex-direction:column;gap:16px">${['Loved by customers everywhere', '100% natural ingredients used', 'Durable for long-term use', 'Best seller this season'].map((t) => `<div class="tlirow" style="display:flex;align-items:center;gap:12px"><span class="pico" style="display:inline-flex;flex:none">${slIcon('<circle cx="12" cy="12" r="10"/><path d="M8 12l2.5 2.5L16 9"/>', SL_ACCENT, 22)}</span><span class="tlit" style="font-size:15px;color:${SL_INK}">${t}</span></div>`).join('')}</div>`) },
  { id: 'benefits-row', label: 'Benefits', html: slWrap(`<div class="benr" style="display:flex;flex-wrap:wrap;gap:44px;justify-content:center;align-items:flex-start">${[[slIcon('<rect x="1" y="6" width="13" height="10" rx="1"/><path d="M14 9h4l3 3v4h-7z"/><circle cx="7" cy="18" r="1.6"/><circle cx="17.5" cy="18" r="1.6"/>', SL_INK, 26), 'Fast Shipping'], [slIcon('<path d="M12 21s-7-4.5-9.2-8.4C1.2 9.3 3 6 6.3 6c1.9 0 3.1 1 5.7 3 2.6-2 3.8-3 5.7-3 3.3 0 5.1 3.3 3.5 6.6C19 16.5 12 21 12 21z"/>', SL_INK, 26), '100% Money Back Guarantee'], [slIcon('<path d="M3 12a9 9 0 1 0 2.6-6.3"/><path d="M3 3.5V8h4.5"/>', SL_INK, 26), 'Free Returns']].map(([ic, t]) => `<div class="benri" style="display:flex;flex-direction:column;align-items:center;gap:9px;max-width:150px;text-align:center"><span class="pico" style="display:inline-flex;color:${SL_INK}">${ic}</span><span class="benrt" style="font-size:14px;font-weight:700;color:${SL_INK}">${t}</span></div>`).join('')}</div>`) },
  { id: 'ingredients-list', label: 'Ingredients List', html: slWrap(`<div class="ingl" style="max-width:520px;margin:0 auto;display:flex;flex-direction:column;gap:14px">${[['🥦', 'Whole vegetables in their natural form'], ['🍓', 'Naturally sourced fresh fruits'], ['🍋', 'Fresh lemon extract from real lemons'], ['🌿', 'Sweet greens and carefully selected herbs']].map(([i, t]) => `<div class="inglrow" style="display:flex;align-items:center;gap:12px"><span class="pico" style="width:34px;height:34px;border-radius:50%;background:${SL_WASH};display:inline-flex;align-items:center;justify-content:center;font-size:18px;flex:none">${i}</span><span class="inglt" style="font-size:15px;color:${SL_INK}">${t}</span></div>`).join('')}</div>`) },
...
function rawLabelFor(el: HTMLElement): string {
  ...
  if (el.classList?.contains('ph')) return 'Image'   // an unset image placeholder is still an "Image" leaf (editable)
...
function buildRawOutline(html: string): RawOutlineNode[] {
  ...
  const walk = (el: HTMLElement, path: number[], depth: number): RawOutlineNode | null => {
    ...
    // (Group (Horizontal/Vertical) → Icon / Text / …), matching PagePilot's deep tree. An image-only wrapper
    // collapses to one "Image" leaf — UNLESS it's a NAMED group (e.g. "Right Group"), which must show its
    // Image child so you can select the image separately (matches PagePilot's Right Group → Image).
    const atomic = cur.classList?.contains('pays') || cur.classList?.contains('payicon') || RAW_ATOMIC_LABELS.has(label) || (rawIsImg(cur) && !isNamed(cur) && cur.tagName === 'IMG')
...
[data-sel="1"]{outline:2px solid ${ORANGE} !important;outline-offset:-2px}
  ...
  // An icon holder = a known icon slot (.t/.ic/.chip/.slchip/.pico) OR any element that is just an svg (a bare
  // icon leaf). This drives PagePilot's Icon panel (Type / picker / size / variant / colour / background).
  const ICON_HOLDER_SEL = '.t, .slchip, .chip, .ic, .pico'
  const isSvgOnly = (el: HTMLElement | null) => !!el && !( el.textContent || '').trim() && el.children.length >= 1 && Array.from(el.children).every((c) => c.tagName === 'svg' || c.tagName === 'SVG')
  const iconHolderEl = useCallback((): HTMLElement | null => {
    const n = rawNodeEl(); if (!n) return null
    if (/^(t|ic|chip|slchip|pico)$/.test(n.className) || isSvgOnly(n)) return n
    return (n.querySelector(ICON_HOLDER_SEL) || (n.closest('.c, .hchecks .c, .slchip') ? n.closest('.c, .slchip')?.querySelector(ICON_HOLDER_SEL) : null)) as HTMLElement | null
  }, [rawNodeEl])
  ...
  // The current icon's rendered SVG (so the panel's compact Icon field previews what's actually there — even a
  ...
  const withIconHolder = useCallback((fn: (h: HTMLElement) => void) => {
    ...
    rawEditHtml(rawSel.ref, (box) => {
      let node: HTMLElement = box
      for (const i of rawSel.path) { const k = node.children[i] as HTMLElement | undefined; if (!k) { node = box; break } node = k }
      const h = (/^(t|ic|chip|slchip|pico)$/.test(node.className) || (!(node.textContent || '').trim() && node.children.length >= 1 && Array.from(node.children).every((c) => /svg/i.test(c.tagName))) ? node : (node.querySelector(ICON_HOLDER_SEL) || node.closest('.c, .slchip')?.querySelector(ICON_HOLDER_SEL))) as HTMLElement | null
      if (h) fn(h)
    })
  }, [rawSel, rawEditHtml])
  ...
  // Variant Picker STYLE mode (PagePilot: Buttons / Dropdowns / Swatches) + selected-state + per-swatch colour.
  const vpickMode = useCallback((): 'buttons' | 'dropdowns' | 'swatches' => { const v = vpickEl(); return v?.classList.contains('swatch') ? 'swatches' : v?.classList.contains('list') ? 'dropdowns' : 'buttons' }, [vpickEl])
  const setVpickMode = useCallback((mode: string) => editVpick((v) => { v.classList.toggle('list', mode === 'dropdowns'); v.classList.toggle('swatch', mode === 'swatches') }), [editVpick])
  const vpickSelVal = useCallback((prop: string): string => { const o = vpickEl()?.querySelector('.vopt.on') as HTMLElement | null; return o?.style.getPropertyValue(prop) || '' }, [vpickEl])
  const setVpickSelStyle = useCallback((prop: string, val: string) => editVpick((v) => v.querySelectorAll('.vopt.on').forEach((o) => { if (val) (o as HTMLElement).style.setProperty(prop, val); else (o as HTMLElement).style.removeProperty(prop) })), [editVpick])
  const vpickSwatchColors = useCallback((): string[] => { const v = vpickEl(); return v ? Array.from(v.querySelectorAll('.vopt')).map((o) => (o as HTMLElement).style.getPropertyValue('--sw') || '') : [] }, [vpickEl])
  const setVpickSwatchColor = useCallback((i: number, color: string) => editVpick((v) => { const o = v.querySelectorAll('.vopt')[i] as HTMLElement | undefined; if (o) { if (color) o.style.setProperty('--sw', color); else o.style.removeProperty('--sw') } }), [editVpick])
  ...
  // Reorder a gallery image within its strip (drag-free ↑/↓, PagePilot-style sequence change).
  const rawGalleryMove = useCallback((path: number[], dir: 'up' | 'down') => { if (rawSel) rawApplyAt(rawSel.ref, path, dir) }, [rawSel, rawApplyAt])
  const rawSetImg = useCallback((url: string) => { if (rawSel) rawApplyAt(rawSel.ref, rawSel.path, 'img', url) }, [rawSel, rawApplyAt])
  // Current image src of the selected piece → the Image panel previews what's actually there (matches PagePilot).
  const rawImgSrc = useCallback((): string => { const n = rawNodeEl(); const im = (n?.tagName === 'IMG' ? n : n?.querySelector('img')) as HTMLImageElement | null; return im?.getAttribute('src') || '' }, [rawNodeEl])
        ...
        <aside style={{ width: 300, borderLeft: `1px solid ${LINE}`, background: '#fff', overflowY: 'auto', padding: 16, display: preview ? 'none' : undefined }}>
          {rawSel ? (
            <RawElementSettings key={rawSel.path.join('.')} name={rawName()} text={rawText()} onText={rawSetText} isImg={rawSel.isImg}
              ...
              gallery={rawGallery()} onGalleryAdd={rawGalleryAdd} onGalleryRemove={rawGalleryRemove} onGalleryReplace={rawGalleryReplace} onGalleryMove={rawGalleryMove} onSetImg={rawSetImg} imgSrc={rawImgSrc()} onImageAI={imageCreateAI} uploadImage={uploadImage}
              ...
              vpickMode={vpickMode()} onVpickMode={setVpickMode} vpickSelVal={vpickSelVal} onVpickSel={setVpickSelStyle} vpickSwatchColors={vpickSwatchColors()} onVpickSwatchColor={setVpickSwatchColor}
...
const SECTION_CAT: Record<string, string> = {
  // Social Proof & Trust = PagePilot's exact 7 (As Seen On, Trusted by thousands, Video Testimonials,
  // Featured Reviews Carousel, Reviews Number, Featured Review, Review Card).
  'as-seen-on': 'Social Proof & Trust', 'trusted-customers': 'Social Proof & Trust', 'video-testimonials': 'Social Proof & Trust', reviews: 'Social Proof & Trust', 'reviews-number': 'Social Proof & Trust', 'featured-review': 'Social Proof & Trust', 'review-card': 'Social Proof & Trust',
  // Trust & Credibility = PagePilot's exact 7.
  'guarantee-90': 'Trust & Credibility', 'money-back': 'Trust & Credibility', 'brand-trust': 'Trust & Credibility', trustpilot: 'Trust & Credibility', 'rated-by': 'Trust & Credibility', bestseller: 'Trust & Credibility', 'payment-icons': 'Trust & Credibility',
  'trust-icons': 'Shipping', 'reviews-photos': 'Image & Content', 'trusted-by': 'Image & Content',
  // Benefits & Features = PagePilot's exact 4.
  'spotlight-benefit': 'Benefits & Features', 'text-list-icons': 'Benefits & Features', 'benefits-row': 'Benefits & Features', 'ingredients-list': 'Benefits & Features',
  // PagePilot puts the comparison table in Layout; our other big content sections go to Image & Content.
  comparison: 'Layout', 'rotating-benefits': 'Image & Content', statistics: 'Image & Content', 'feature-cards': 'Image & Content', numbered: 'Image & Content',
...
const SECTION_CAT_ORDER = ['Social Proof & Trust', 'Trust & Credibility', 'Benefits & Features', 'Image & Content', 'Conversion / CTA', 'Layout', 'FAQ', 'Guarantee']
...
function RawElementSettings({ name, text, onText, isImg, isText, html, onHtml, textContext, isLink, href, onHref, gallery, onGalleryAdd, onGalleryRemove, onGalleryReplace, onGalleryMove, onSetImg, imgSrc, onImageAI, uploadImage, isGallery, sticky, onSticky, onCreateAI, aiBusy, galleryVal, onGalleryPart, isPays, paysActive, onTogglePay, paysAlign, paysGap, onPaysAlign, onPaysGap, isRing, ringPct, ringSize, ringDur, onRingPct, onRingSize, onRingDur, isLogo, logoImg, onLogoImage, isAcc, accRows, onAccAdd, onAccRemove, isVpick, vpickList, vpickOpts, onVpickStyle, onVpickAdd, onVpickRemove, vpickStyleVal, onVpickTextStyle, vpickGap, onVpickGap, vpickMode, onVpickMode, vpickSelVal, onVpickSel, vpickSwatchColors, onVpickSwatchColor, isCart, cart, onCart, isSave, saveMode, saveShow, onSaveBadge, isPrice, priceVal, onPrice, isDivider, isReviews, reviewsVal, onReviews, isIconItem, itemIcon, onItemIcon, iconSize, onIconSize, iconStyleVal, onIconStyle, iconCurrentSvg, onIconBgSize, urlOpen, onUrlOpen, device, onDevice, onRename, hasChildren, getVal, onStyle, onOp, onClear }: { name: string; text: string; onText: (t: string) => void; isImg: boolean; isText: boolean; html: string; onHtml: (h: string) => void; textContext: string; isLink: boolean; href: string; onHref: (u: string) => void; isRing: boolean; ringPct: string; ringSize: string; ringDur: string; onRingPct: (v: string) => void; onRingSize: (v: string) => void; onRingDur: (v: string) => void; isLogo: boolean; logoImg: string; onLogoImage: (u: string) => void; isAcc: boolean; accRows: string[]; onAccAdd: () => void; onAccRemove: (i: number) => void; isVpick: boolean; vpickList: boolean; vpickOpts: string[]; onVpickStyle: (list: boolean) => void; onVpickAdd: () => void; onVpickRemove: (i: number) => void; vpickStyleVal: (prop: string) => string; onVpickTextStyle: (prop: string, v: string) => void; vpickGap: string; onVpickGap: (v: string) => void; vpickMode: 'buttons' | 'dropdowns' | 'swatches'; onVpickMode: (m: string) => void; vpickSelVal: (prop: string) => string; onVpickSel: (prop: string, v: string) => void; vpickSwatchColors: string[]; onVpickSwatchColor: (i: number, color: string) => void; isCart: boolean; cart: { icon: string; label: string; show: boolean; pos: 'left' | 'right'; size: number }; onCart: (patch: Partial<{ icon: string; label: string; show: boolean; pos: 'left' | 'right'; size: number }>) => void; isSave: boolean; saveMode: 'percent' | 'value'; saveShow: boolean; onSaveBadge: (patch: Partial<{ mode: 'percent' | 'value'; show: boolean }>) => void; isPrice: boolean; priceVal: (part: string, prop: string) => string; onPrice: (part: string, prop: string, val: string) => void; isDivider: boolean; isReviews: boolean; reviewsVal: (part: string, prop: string) => string; onReviews: (part: string, prop: string, val: string) => void; isIconItem: boolean; itemIcon: string; onItemIcon: (icon: string) => void; iconSize: string; onIconSize: (px: string) => void; iconStyleVal: (p: string) => string; onIconStyle: (p: string, v: string) => void; iconCurrentSvg: string; onIconBgSize: (px: string) => void; device: Device; onDevice: (d: Device) => void; onRename: (name: string) => void; hasChildren: boolean; gallery: { src: string; path: number[] }[] | null; onGalleryAdd: (url: string) => void; onGalleryRemove: (path: number[]) => void; onGalleryReplace: (path: number[], url: string) => void; onGalleryMove: (path: number[], dir: 'up' | 'down') => void; onSetImg: (url: string) => void; imgSrc: string; onImageAI: () => void; uploadImage: (f: File) => Promise<string | null>; isGallery: boolean; sticky: boolean; onSticky: (v: boolean) => void; onCreateAI: () => void; aiBusy: boolean; galleryVal: (part: string, prop: string) => string; onGalleryPart: (part: string, prop: string, val: string) => void; isPays: boolean; paysActive: string[]; onTogglePay: (id: string) => void; paysAlign: string; paysGap: string; onPaysAlign: (v: string) => void; onPaysGap: (v: string) => void; urlOpen: boolean; onUrlOpen: (v: boolean) => void; getVal: (p: string) => string; onStyle: (p: string, v: string) => void; onOp: (op: RawOp) => void; onClear: () => void }) {
  ...
  const isMedia = isImg || gallery != null || isGallery || isPays || isRing || isLogo || isVpick || isAcc || isCart || isSave || isPrice || isDivider || isReviews || isIconItem
    ...
    <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 10 }}>
            {gallery.map((g, gi) => (
              <div key={g.path.join('.')} style={{ position: 'relative', paddingTop: '100%', borderRadius: 10, overflow: 'hidden', border: `1px solid ${LINE}`, background: '#faf9f7' }}>
                ...
                {/* Reorder the image sequence (matches PagePilot's drag-to-reorder, done with ↑/↓ here). */}
                <div style={{ position: 'absolute', left: 4, top: 4, display: 'flex', gap: 3 }}>
                  {gi > 0 && <button title="Move left" onClick={() => onGalleryMove(g.path, 'up')} style={{ border: 0, background: 'rgba(20,18,15,.72)', color: '#fff', borderRadius: 6, fontSize: 12, width: 22, height: 22, cursor: 'pointer', lineHeight: 1 }}>‹</button>}
                  {gi < gallery.length - 1 && <button title="Move right" onClick={() => onGalleryMove(g.path, 'down')} style={{ border: 0, background: 'rgba(20,18,15,.72)', color: '#fff', borderRadius: 6, fontSize: 12, width: 22, height: 22, cursor: 'pointer', lineHeight: 1 }}>›</button>}
                </div>
              </div>
            ))}
          </div>
      ...
      {isVpick && (
        <div style={{ marginBottom: 14 }}>
          ...
          <SegRow label="Style" prop="__vstyle" options={[['buttons', 'Buttons'], ['dropdowns', 'Dropdowns'], ['swatches', 'Swatches']]} getVal={() => vpickMode} onStyle={(_p, v) => onVpickMode(v)} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, margin: '8px 0' }}>
            {vpickOpts.map((o, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, border: `1px solid ${LINE}`, borderRadius: 9, padding: '7px 10px' }}>
                {vpickMode === 'swatches' && <input type="color" value={/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(vpickSwatchColors[i] || '') ? vpickSwatchColors[i] : '#cccccc'} onChange={(e) => onVpickSwatchColor(i, e.target.value)} title="Swatch color" style={{ width: 24, height: 24, flex: 'none', border: `1px solid ${LINE}`, borderRadius: 6, padding: 0, cursor: 'pointer', background: 'none' }} />}
          ...
          <div style={{ fontSize: 11, color: FAINT, margin: '6px 0 12px' }}>{vpickMode === 'swatches' ? 'Set each option’s swatch color above. Double-click an option on the canvas to rename it.' : 'Double-click an option on the canvas to rename it.'}</div>
          <div style={{ borderTop: `1px solid ${LINE}`, paddingTop: 14, marginTop: 4 }}>
            <div style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 8, color: INK }}>Variants</div>
            <ColorField label="Text color" prop="color" getVal={vpickStyleVal} onStyle={onVpickTextStyle} />
            <NumRow label="Size" prop="font-size" min={10} max={24} getVal={vpickStyleVal} onStyle={onVpickTextStyle} />
            ...
            <ColorField label="Background" prop="background-color" getVal={vpickStyleVal} onStyle={onVpickTextStyle} />
            <SelRow label="Border style" prop="border-style" options={[['solid', 'Solid'], ['dashed', 'Dashed'], ['dotted', 'Dotted'], ['none', 'None']]} getVal={vpickStyleVal} onStyle={onVpickTextStyle} />
            <ColorField label="Border color" prop="border-color" getVal={vpickStyleVal} onStyle={onVpickTextStyle} />
            <NumRow label="Rounded corners" prop="border-radius" max={40} getVal={vpickStyleVal} onStyle={onVpickTextStyle} />
          </div>
          <div style={{ borderTop: `1px solid ${LINE}`, paddingTop: 14, marginTop: 14 }}>
            <div style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 8, color: INK }}>Selected variant</div>
            <ColorField label="Text color" prop="color" getVal={vpickSelVal} onStyle={onVpickSel} />
            <ColorField label="Background" prop="background-color" getVal={vpickSelVal} onStyle={onVpickSel} />
            <ColorField label="Border color" prop="border-color" getVal={vpickSelVal} onStyle={onVpickSel} />
          </div>
        </div>
      )}
      {singleImg && (
      ...
      {isIconItem && !isGroup && (
        <div style={{ marginBottom: 14 }}>
          <SegRow label="Type" prop="__ictype" options={[['icon', 'Icon'], ['emoji', 'Emoji'], ['svg', 'SVG']]} getVal={() => (itemIcon && LINE_ICONS[itemIcon] ? 'icon' : (itemIcon ? 'emoji' : 'icon'))} onStyle={() => {}} />
          <IconField value={itemIcon} currentSvg={iconCurrentSvg} onPick={onItemIcon} />
          <NumRow label="Icon size" prop="__icsize" min={10} max={64} unit="px" getVal={() => iconSize} onStyle={(_p, v) => onIconSize(v)} />
          <SegRow label="Icon variant" prop="stroke-width" options={[['1.4', 'Light'], ['1.8', 'Regular'], ['2.4', 'Bold']]} getVal={() => { const w = iconStyleVal('stroke-width'); return w || '1.8' }} onStyle={(p, v) => onIconStyle(p, v)} />
...
function treeIconFor(label: string, hasChildren?: boolean): React.ReactNode {
  ...
  if (/^icon$/.test(l)) return ic(<path d="M12 3l2.4 4.9 5.4.8-3.9 3.8.9 5.4-4.8-2.5-4.8 2.5.9-5.4L4.2 8.7l5.4-.8z" />)   // sparkle/star — icon type
  if (/^text$|^paragraph$|tagline|^quote$|reviewer|^subhead/.test(l)) return ic(<path d="M4 6h16M4 11h12M4 16h15" />)   // paragraph lines — text type
  if (hasChildren || /group|row|section|creative|buy box|product details|gallery|container|rotating content/.test(l)) return ic(<><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /></>)
