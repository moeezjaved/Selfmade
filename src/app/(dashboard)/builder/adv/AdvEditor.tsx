...
const slPays = () => `<div class="pays" style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;align-items:center;margin-top:18px">${[
  ...
  slPayCard(`<span style="display:inline-flex;align-items:center;gap:2px;font-weight:600;font-size:11px;color:#111"><svg width="10" height="12" viewBox="0 0 14 17" fill="#111"><path d="M11.2 9c0-1.9 1.5-2.8 1.6-2.9-.9-1.3-2.3-1.5-2.7-1.5-1.2-.1-2.3.7-2.9.7-.6 0-1.5-.7-2.5-.7-1.3 0-2.5.7-3.1 1.9-1.3 2.3-.3 5.7 1 7.6.6.9 1.3 1.9 2.3 1.9.9 0 1.3-.6 2.4-.6s1.4.6 2.4.6 1.6-.9 2.2-1.8c.7-1 1-2 1-2.1-.1 0-1.9-.7-1.9-2.6zM9.4 3.4c.5-.6.9-1.5.8-2.4-.8 0-1.7.5-2.3 1.2-.5.6-.9 1.5-.8 2.3.9.1 1.7-.5 2.3-1.1z"/></svg>Pay</span>`),
  ...
  slPayCard(`<span style="color:#5a31f4;font-weight:900;font-size:12px">shop</span>`),
].join('')}</div>`
...
const SECTION_LIBRARY: { id: string; label: string; html: string }[] = [
  ...
  { id: 'guarantee-90', label: '90-Day Money Back Guarantee', html: slWrap(`<div style="text-align:center"><div class="g90" style="display:inline-flex;align-items:center;gap:14px;background:#fff;border:1px solid ${SL_LINE};border-radius:14px;padding:16px 24px;box-shadow:0 3px 14px -8px rgba(20,18,15,.2);text-align:left"><span class="pico" style="display:inline-flex;flex:none">${SL_SHIELD('#1ea672')}</span><div><div class="g90t" style="font-size:16px;font-weight:800;color:${SL_INK}">90-Day Money Back Guarantee</div><div class="g90s" style="font-size:13.5px;color:${SL_SUB}">Full refund, no questions asked.</div></div></div></div>`) },
  { id: 'money-back', label: 'Money Back Guarantee', html: slWrap(`<div class="mbg" style="display:flex;align-items:center;gap:18px;max-width:660px;margin:0 auto;background:#fff;border:1px solid ${SL_LINE};border-radius:16px;padding:22px 24px;box-shadow:0 3px 14px -8px rgba(20,18,15,.2)"><span class="pico" style="display:inline-flex;flex:none">${SL_SHIELD(SL_ACCENT)}</span><div><div class="mbt" style="font-size:16px;font-weight:800;color:${SL_INK};display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span style="background:#fdeee9;color:#d64316;font-size:11px;font-weight:800;padding:2px 8px;border-radius:6px">NO RISK</span>100% Money Back Guarantee</div><p class="mbs" style="font-size:14px;color:${SL_SUB};line-height:1.6;margin:6px 0 0">We get it — you've been disappointed before. If you're not happy, we'll refund every penny. No hoops, no hassle.</p></div></div>`) },
  { id: 'brand-trust', label: 'Brand Trust', html: slWrap(`<div class="btrust" style="display:flex;flex-wrap:wrap;gap:34px;justify-content:center;align-items:center">${[['🇺🇸', 'Made in the USA'], ['🏆', 'Award Winning Product'], ['🌿', 'Clean Ingredients']].map(([i, t]) => `<div class="bti" style="display:flex;align-items:center;gap:10px;font-size:15px;font-weight:700;color:${SL_INK}"><span style="font-size:22px">${i}</span>${t}</div>`).join('')}</div>`) },
  { id: 'trustpilot', label: 'Trustpilot Rating', html: slWrap(`<div class="tpilot" style="display:flex;align-items:center;justify-content:center;gap:12px;flex-wrap:wrap"><span style="font-size:20px;font-weight:800;color:${SL_INK}">Excellent</span><span style="display:inline-flex;gap:3px">${[0, 0, 0, 0, 0].map(() => `<span style="width:26px;height:26px;background:#00b67a;display:inline-flex;align-items:center;justify-content:center;color:#fff;font-size:16px;border-radius:3px">★</span>`).join('')}</span><span style="font-size:17px;font-weight:800;color:${SL_INK};display:inline-flex;align-items:center;gap:4px"><span style="color:#00b67a">★</span>Trustpilot</span></div>`) },
  { id: 'rated-by', label: 'Rated by', html: slWrap(`<div class="ratedby" style="text-align:center;font-size:15px;color:${SL_INK}"><span class="stars" style="color:#f5a623;letter-spacing:2px;font-size:19px;vertical-align:middle">★★★★★</span> <span class="rbt">Rated <b>4.9/5</b> based on <b>2,453+</b> Trusted Reviews</span></div>`) },
  { id: 'bestseller', label: 'Best Seller Badge', html: slWrap(`<div style="text-align:center"><div class="bsb" style="display:inline-flex;align-items:center;gap:12px;background:#fff;border:1px solid ${SL_LINE};border-radius:12px;padding:12px 18px;box-shadow:0 3px 14px -8px rgba(20,18,15,.2);text-align:left"><span style="background:${SL_ACCENT};color:#fff;font-weight:900;font-size:15px;padding:6px 11px;border-radius:8px;flex:none">#1</span><div><div class="bsbt" style="font-size:14px;font-weight:800;color:${SL_INK};letter-spacing:.03em">BESTSELLER OF 2026</div><div class="bsbs" style="font-size:13px;color:${SL_SUB}">Trusted by thousands worldwide</div></div></div></div>`) },
