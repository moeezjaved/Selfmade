...
const SL_VIDEO = svgURI(`<svg xmlns="http://www.w3.org/2000/svg" width="360" height="520"><defs><linearGradient id="v" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#e2e6fb"/><stop offset="1" stop-color="#b4bdf3"/></linearGradient></defs><rect width="360" height="520" fill="url(#v)"/><circle cx="180" cy="245" r="30" fill="#fff" opacity=".6"/></svg>`)
...
const SECTION_LIBRARY: { id: string; label: string; html: string }[] = [
  ...
  { id: 'trusted-customers', label: 'Trusted by thousands customers', html: slWrap(`<div class="tbc" style="display:flex;align-items:center;justify-content:center;gap:16px;flex-wrap:wrap"><div class="avs" style="display:flex">${[0, 1, 2, 3].map((i) => `<span class="avim" style="width:46px;height:46px;border-radius:50%;border:2px solid #fff;margin-left:${i ? -14 : 0}px;overflow:hidden;flex:none;box-shadow:0 1px 4px rgba(20,18,15,.15)"><img src="${SL_AVATAR}" alt="" style="width:100%;height:100%;object-fit:cover"></span>`).join('')}</div><div style="text-align:left"><div class="tbt" style="font-size:17px;font-weight:800;color:${SL_INK}">Trusted by thousands of customers</div><div class="tbs" style="font-size:14px;color:${SL_SUB}">They choose us for premium quality and a satisfaction guarantee.</div></div></div>`) },
  { id: 'video-testimonials', label: 'Video Testimonials Carousel', html: slWrap(slHead('They tried it. They adopted it.') + `<div class="vcar" style="display:flex;gap:14px;overflow-x:auto;padding:4px 2px;scrollbar-width:none">${[1, 2, 3, 4].map(() => `<div class="vcard" style="flex:0 0 210px;max-width:210px;position:relative;border-radius:16px;overflow:hidden;aspect-ratio:9/13"><img src="${SL_VIDEO}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"><span style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:54px;height:54px;border-radius:50%;background:rgba(255,255,255,.92);display:flex;align-items:center;justify-content:center;font-size:20px;color:${SL_ACCENT};padding-left:4px">▶</span></div>`).join('')}</div>`) },
