...
const INK = '#141d15', SUB = '#7a9a7a', LIME = '#ff5a2c', FOREST = '#141d15', LINE = 'rgba(0,0,0,0.07)'
    ...
    <div style={{padding:'32px 28px',maxWidth:980,margin:'0 auto'}}>
      ...
      <div style={{background:'#ffffff',border:'1px solid rgba(0,0,0,0.07)',borderRadius:18,overflow:'hidden'}}>
        <div style={{padding:'18px 22px',borderBottom:'1px solid rgba(255,90,44,0.08)'}}>
          <div style={{fontSize:15,fontWeight:700,color:'#141d15'}}>Profile</div>
        </div>
        <div style={{padding:22,display:'flex',flexDirection:'column',gap:14}}>
          <div>
            ...
            <input
              ...
              style={{width:'100%',padding:'10px 14px',borderRadius:10,border:'1.5px solid rgba(255,255,255,0.1)',background:'#f9f5ec',color:'#141d15',fontSize:14,fontFamily:'inherit',outline:'none'}}
            />
          </div>
          <div>
            ...
            <input
              ...
              style={{width:'100%',padding:'10px 14px',borderRadius:10,border:'1.5px solid rgba(255,255,255,0.06)',background:'#f9f5ec',color:'#7a9a7a',fontSize:14,fontFamily:'inherit',outline:'none',cursor:'not-allowed'}}
            />
          </div>
          <button
            onClick={saveProfile}
      ...
      <div style={{background:'#ffffff',border:'1px solid rgba(0,0,0,0.07)',borderRadius:18,overflow:'hidden'}}>
        <div style={{padding:'18px 22px',borderBottom:'1px solid rgba(255,90,44,0.08)'}}>
          <div style={{fontSize:15,fontWeight:700,color:'#141d15'}}>Change Password</div>
        </div>
        <div style={{padding:22,display:'flex',flexDirection:'column',gap:14}}>
          <div>
      ...
      <div style={{background:'#ffffff',border:'1px solid rgba(0,0,0,0.07)',borderRadius:18,overflow:'hidden'}}>
        ...
        <div style={{padding:12}}>
          ...
          <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',padding:'6px 4px 14px'}}>
            <select value={apBrand} onChange={(e)=>setApBrand(e.target.value)} style={{flex:'1 1 160px',minWidth:150,padding:'9px 12px',borderRadius:10,border:'1.5px solid #e2e8f0',background:'#fff',fontSize:13,fontFamily:'inherit',color:'#141d15'}}>
              <option value="">{brands.length ? 'Choose a brand…' : 'Add a brand first'}</option>
              {brands.map(b=> <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
            <select value={apMedia} onChange={(e)=>setApMedia(e.target.value as any)} style={{padding:'9px 12px',borderRadius:10,border:'1.5px solid #e2e8f0',background:'#fff',fontSize:13,fontFamily:'inherit',color:'#141d15'}}>
              <option value="image">Image ad</option>
              <option value="video">Video ad</option>
            </select>
            {/* Optional: follow ONE competitor from Brand Spy — the daily ad is modelled on their latest ad. */}
            <select value={apComp} onChange={(e)=>setApComp(e.target.value)} title="Reference a competitor's ads" style={{flex:'1 1 160px',minWidth:150,padding:'9px 12px',borderRadius:10,border:'1.5px solid #e2e8f0',background:'#fff',fontSize:13,fontFamily:'inherit',color:'#141d15'}}>
      ...
      <div style={{background:'#ffffff',border:'1px solid rgba(0,0,0,0.07)',borderRadius:18,overflow:'hidden'}}>
        ...
        <div style={{padding:22,display:'flex',flexDirection:'column',gap:4}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 0',gap:12}}>
            <div>
              <div style={{fontSize:14,fontWeight:600,color:'#141d15'}}>Email digest</div>
            ...
            <select
              ...
              style={{padding:'7px 12px',borderRadius:8,border:'1.5px solid #e2e8f0',background:'#f9f5ec',color:'#141d15',fontSize:13,fontFamily:'inherit',fontWeight:600,cursor:'pointer'}}
            >
      ...
      <div style={{background:'#ffffff',border:'1px solid rgba(0,0,0,0.07)',borderRadius:18,overflow:'hidden'}}>
        <div style={{padding:'18px 22px',borderBottom:'1px solid rgba(255,90,44,0.08)'}}>
          <div style={{fontSize:15,fontWeight:700,color:'#141d15'}}>Save ads anywhere</div>
        ...
        <div style={{padding:22,display:'flex',flexDirection:'column',gap:16}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:14,flexWrap:'wrap'}}>
            ...
            <div style={{flex:1,minWidth:180}}>
              <div style={{fontSize:14,fontWeight:700,color:'#141d15'}}>Chrome extension</div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:14,flexWrap:'wrap',opacity:0.85}}>
            ...
            <div style={{flex:1,minWidth:180}}>
              <div style={{fontSize:14,fontWeight:700,color:'#141d15'}}>Save from Instagram on mobile</div>
      ...
      <div style={{background:'#ffffff',border:'1px solid rgba(0,0,0,0.07)',borderRadius:18,overflow:'hidden'}}>
        <div style={{padding:'18px 22px',borderBottom:'1px solid rgba(255,90,44,0.08)'}}>
          <div style={{fontSize:15,fontWeight:700,color:'#141d15'}}>Connected Accounts</div>
        </div>
        <div style={{padding:22}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:16}}>
              {metaConnected ? (
                <>
                  <a href="/connect-meta" style={{fontSize:13,color:'#141d15',fontWeight:700,textDecoration:'none'}}>Reconnect →</a>
...
function ChannelsSection() {
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 180 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#141d15' }}>{label}{c && <span style={{ fontSize: 11, fontWeight: 700, color: '#3b6d11', background: '#eaf3de', borderRadius: 20, padding: '2px 8px', marginLeft: 8 }}>Connected ✓</span>}</div>
  ...
  return (
    <div style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 18, overflow: 'hidden', marginBottom: 16 }}>
      <div style={{ padding: '18px 22px', borderBottom: '1px solid rgba(255,90,44,0.08)' }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#141d15' }}>Mello on Slack <span style={{ fontSize: 11, fontWeight: 700, color: '#3b6d11', background: '#eaf3de', borderRadius: 20, padding: '2px 8px', marginLeft: 6 }}>your personal line</span></div>
...
function CustomerChannelsSection() {
  ...
  const Row = ({ provider, label, logo, how }: { provider: string; label: string; logo: ReactNode; how: string }) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
        <LogoTile>{logo}</LogoTile>
        <div style={{ flex: 1, minWidth: 180 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#141d15' }}>{label}{on && <span style={{ fontSize: 11, fontWeight: 700, color: '#3b6d11', background: '#eaf3de', borderRadius: 20, padding: '2px 8px', marginLeft: 8 }}>Connected ✓</span>}</div>
          <div style={{ fontSize: 12.5, color: '#7a9a7a', marginTop: 2 }}>{how}</div>
        </div>
        {on ? (
  ...
  return (
    <div style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 18, overflow: 'hidden' }}>
      <div style={{ padding: '18px 22px', borderBottom: '1px solid rgba(255,90,44,0.08)' }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#141d15' }}>Your customer channels</div>
      ...
      <div style={{ padding: '18px 22px', borderTop: '1px solid #f1f5f9', borderBottom: '1px solid rgba(255,90,44,0.08)' }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#141d15' }}>Your calendar</div>
...
function ToggleRow({ label, hint, checked, onChange }: { label: string; hint: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 0',gap:12}}>
      <div>
        <div style={{fontSize:14,fontWeight:600,color:'#141d15'}}>{label}</div>
        <div style={{fontSize:12,color:'#7a9a7a',marginTop:2}}>{hint}</div>
      </div>
      <button
        role="switch" aria-checked={checked} onClick={() => onChange(!checked)}
        style={{width:44,height:26,borderRadius:100,border:'none',cursor:'pointer',flexShrink:0,padding:0,position:'relative',background:checked?'#141d15':'#d1d5db',transition:'background .15s'}}
      >
