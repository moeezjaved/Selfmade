                    ...
                    <div key={cat.name} style={{ marginTop: 20 }}>
                      ...
                      {!off && (
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 10 }}>
                          {cat.tiles.map((t) => (
                            <button key={t.label} type="button" disabled={t.soon} onClick={() => { if (t.soon) return; const dest = t.seed && t.href.startsWith('/ads-workspace') ? `${t.href}?idea=${encodeURIComponent(t.seed)}` : t.href; router.push(dest) }} title={t.soon ? 'Coming soon' : undefined} style={{ position: 'relative', background: '#fff', border: `1px solid ${LINE}`, borderRadius: 13, padding: 14, textAlign: 'left', width: '100%', cursor: t.soon ? 'default' : 'pointer', opacity: t.soon ? 0.65 : 1, font: 'inherit', color: INK, boxShadow: '0 1px 2px rgba(20,18,15,.05)' }}>
