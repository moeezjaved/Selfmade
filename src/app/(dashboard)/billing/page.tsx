...
export default function BillingPage() {
  return (
    <div style={{padding:28,maxWidth:600}}>
      <h1 style={{fontSize:22,fontWeight:800,color:'#1a3a1a',marginBottom:6}}>Billing</h1>
      <p style={{fontSize:13,color:'#7a9a7a',marginBottom:24}}>Manage your subscription and payment method.</p>
      <div style={{background:'#152928',border:'1px solid rgba(223,254,149,0.22)',borderRadius:20,padding:32,position:'relative',overflow:'hidden'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:24}}>
          {['Live KPI Dashboard','AI Recommendations','Approval-First','Ad Engine','Creative Studio','Activity Log'].map(f => (
            <div key={f} style={{display:'flex',alignItems:'center',gap:7,fontSize:13,color:'#5a7a5a'}}><span style={{color:'#1a3a1a',fontWeight:900}}>✓</span>{f}</div>
          ))}
        </div>
