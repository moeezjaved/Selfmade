...
type Item = { id?: string; kind: string; importance: number; title: string; body?: string; why?: string; cta_label?: string; cta_href?: string; media?: { image: string | null; videoUrl: string | null; adId?: string }[]; at?: string }
type Brief = {
      ...
      <div className="bs-grid">
        ...
        <div className="bs-col">
          ...
          {competitors.length === 0
            ...
            : competitors.map((c, i) => (
                <Row key={c.id || i} first={i === 0} right={ARROW} onClick={() => onAct(c)}
                  href={c.cta_href?.replace('/knowledge/brand/', '/discovery/brand-spy/') || c.cta_href}
                  title={c.title.replace(/\s+launched.*$/i, '').replace(/[.:]$/, '')}
                  desc={(c.title.match(/launched.*/i)?.[0] || 'New activity').replace(/\.$/, '') + (ago(c.at) ? ` · ${ago(c.at)}` : '')} />
              ))}
        </div>
