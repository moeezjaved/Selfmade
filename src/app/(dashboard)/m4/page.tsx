...
export default function M4Page() {
  ...
  const UploadBox = ({list,setter,label}:{list:Creative[],setter:React.Dispatch<React.SetStateAction<Creative[]>>,label:string}) => (
    <div style={{marginBottom:16}}>
      ...
      <label style={{background:'rgba(223,254,149,0.1)',border:'1px solid rgba(223,254,149,0.2)',color:'#dffe95',padding:'8px 16px',borderRadius:100,fontSize:13,fontWeight:700,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:8}}>
        Upload<input type="file" accept="image/*,video/*" multiple onChange={e=>handleUpload(e,setter)} style={{display:'none'}}/>
      </label>
    </div>
  )
  ...
  return (
    <div style={{padding:28,maxWidth:860,margin:'0 auto'}}>
      ...
      {step==='pixel'&&(
        <div style={S.card}>
          <div style={S.head}><div style={{fontSize:15,fontWeight:800,color:'white'}}>Step 1 — Product and Competitor Intelligence</div><div style={{fontSize:13,color:'rgba(255,255,255,0.4)',marginTop:3}}>The more detail you give, the better Claude targets your audience.</div></div>
          <div style={S.body}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
              <div><label style={S.label}>Product Name</label><input value={form.product} onChange={e=>set('product',e.target.value)} placeholder="e.g. HairResQ" style={S.input}/></div>
              <div><label style={S.label}>Campaign Name</label><input value={form.campaignName} onChange={e=>set('campaignName',e.target.value)} placeholder="e.g. Hair Fall M4 May 2025" style={S.input}/></div>
            </div>
            <div style={{marginBottom:14}}><label style={S.label}>What do you sell?</label><textarea value={form.description} onChange={e=>set('description',e.target.value)} placeholder="Describe your product, unique benefits, guarantee..." style={{...S.input,resize:'vertical',minHeight:70,lineHeight:1.6} as React.CSSProperties}></textarea></div>
