...
export default function CampaignsPage() {
  ...
  const toggleStatus = async (id: string, type: 'campaign'|'adset'|'ad', currentStatus: string) => {
    ...
    <div style={{ padding: 28, maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={loadCampaigns} style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.7)', border: 'none', padding: '10px 18px', borderRadius: 100, fontSize: 13, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>
            ↻ Refresh
          </button>
          <button onClick={() => window.location.href = '/m4'} style={{ background: '#dffe95', color: '#10211f', border: 'none', padding: '10px 22px', borderRadius: 100, fontSize: 13, fontWeight: 800, fontFamily: 'inherit', cursor: 'pointer' }}>
            🚀 Launch New
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: '#152928', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 20, padding: 48, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: 'white', marginBottom: 8 }}>No campaigns found</div>
          <button onClick={() => window.location.href = '/m4'} style={{ background: '#dffe95', color: '#10211f', border: 'none', padding: '10px 24px', borderRadius: 100, fontSize: 14, fontWeight: 800, fontFamily: 'inherit', cursor: 'pointer' }}>
              ...
              <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>
                    {camp.status} · {camp.objective?.replace('OUTCOME_', '')} · Budget: {fmt(Math.round((camp.daily_budget || 0) / 100))}/day · {camp.adsets?.length || 0} ad sets
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.3)' }}>{expandedCamp[camp.id] ? '▲' : '▼'}</div>
                </div>
              </div>
                ...
                <div key={adset.id} style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                  <div style={{ padding: '12px 20px 12px 36px', display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.01)', cursor: 'pointer' }}
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)' }}>{expandedAdset[adset.id] ? '▲' : '▼'}</div>
                    </div>
                  </div>
                    ...
                    <div key={ad.id} style={{ padding: '10px 20px 10px 52px', borderTop: '1px solid rgba(255,255,255,0.03)', display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.005)' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => setEditModal({
                          ...
                          style={{ background: 'rgba(223,254,149,0.08)', border: '1px solid rgba(223,254,149,0.15)', color: '#dffe95', padding: '4px 10px', borderRadius: 100, fontSize: 11, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>
                          ✏️ Edit
                        </button>
      ...
      {editModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
          <div style={{ background: '#152928', border: '1px solid rgba(223,254,149,0.2)', borderRadius: 20, width: '100%', maxWidth: 480, padding: 24 }}>

            <div style={{ fontSize: 18, fontWeight: 900, color: 'white', marginBottom: 4 }}>
              {editModal.action === 'update_budget' ? '💰 Edit Budget' : '✏️ Edit Ad'}
            </div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 20 }}>{editModal.name}</div>
            ...
            {editModal.action === 'update_budget' && (
              <div>
                ...
                <input
                  ...
                  style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '12px 14px', color: 'white', fontSize: 16, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }}
                />
              </div>
            )}
            ...
            {editModal.action === 'update_ad' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <Field label="Primary Text" value={editModal.primary_text} onChange={(v: string) => setEditModal((p: any) => ({ ...p, primary_text: v }))} multiline />
                <Field label="Headline" value={editModal.headline} onChange={(v: string) => setEditModal((p: any) => ({ ...p, headline: v }))} />
                <Field label="Destination URL" value={editModal.link_url} onChange={(v: string) => setEditModal((p: any) => ({ ...p, link_url: v }))} />
              </div>
            )}
