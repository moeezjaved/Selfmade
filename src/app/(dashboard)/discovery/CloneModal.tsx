          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, overflow: 'auto' }}>
            <div style={{ padding: 22, borderRight: `1px solid ${L_LINE}` }}>
              ...
              <div style={{ marginTop: 16, fontSize: 13, color: L_MUTED, lineHeight: 1.6 }}>
                Your remake is ready. Keep editing it by just typing on the right — “make the background pink”, “move the logo up”, or change who’s on the image: <b style={{ color: L_INK }}>“make the person Pakistani”</b> (also Indian, Arab, East Asian, Black, White, Hispanic…) — with full undo. Everything is saved in <b style={{ color: L_INK }}>My Creatives</b>.
              </div>
                ...
                {step === 2 && (
                  <section>
                    ...
                    <div className="field">
                      ...
                      {look !== 'match' && <div style={{ fontSize: 11.5, color: L_FAINT, marginTop: 6 }}>💡 We render your ad on Pro, then automatically recast the person as <b>{look}</b> — same pose, product and text. Takes a few extra seconds.</div>}
                    </div>
                  </section>
                )}
                ...
                {step === 4 && (
                  <section>
                    ...
                    <div style={{ border: `1px solid ${L_LINE}`, borderRadius: 16, overflow: 'hidden', marginBottom: 16 }}>
                      ...
                      <ReviewRow k="On the image" v={look === 'match' ? 'Match the original' : `Recast as ${look}`} onEdit={() => setStep(2)} />
...
const chip = (on: boolean): React.CSSProperties => ({ background: on ? '#ef4a1e' : '#fff', color: on ? '#fff' : '#333d35', border: `1.5px solid ${on ? '#ef4a1e' : L_LINE}`, borderRadius: 99, padding: '9px 16px', fontSize: 13, fontWeight: 650, cursor: 'pointer', fontFamily: 'inherit' })
