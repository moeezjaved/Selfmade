        ...
        <div onClick={() => setConfirmSpy(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(14,27,18,0.5)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div onClick={e => e.stopPropagation()} style={{ width: 'min(440px,96vw)', background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 24px 70px rgba(0,0,0,0.35)' }}>
            ...
            <div style={{ fontSize: 13.5, color: '#6b7280', marginTop: 8, lineHeight: 1.55 }}>
              We’ll crawl their full ad archive from the Meta Ad Library and track it over time. This costs <b>50 credits ($0.50)</b> per brand — each spy runs a fresh crawl, so adding a brand back after you remove it costs 50 credits again.
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={() => doSpy(confirmSpy)} style={{ background: '#ef4a1e', color: '#fff', border: 'none', padding: '9px 22px', borderRadius: 100, fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>Spy this brand</button>
            </div>
          </div>
        </div>
      )}
