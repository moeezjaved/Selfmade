...
export default function IndexerAdminPage() {
  ...
  const runCrawler = async (term?: string, country?: string) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setRunLog(prev => [
          ...prev,
          `✅ Terms processed: ${data.termsProcessed?.join(', ')}`,
          `📦 Ads upserted: ${data.totalAdsUpserted}`,
          `🔢 Embeddings generated: ${data.embedded}`,
          `🤖 Claude classified: ${data.classified}`,
          `📊 Total in DB: ${data.totalInDB?.toLocaleString()}`,
          ...(data.log || []).map((l: any) => l.error ? `  ❌ ${l.term}/${l.country}: ${l.error}` : `  ✅ ${l.term}/${l.country}: ${l.ads_fetched} ads`),
        ])
        fetchStats()
      }
    } catch (e: any) {
      setRunLog(prev => [...prev, `❌ ${e.message}`])
    }
    setRunning(false)
  }
  ...
  const filteredTerms = (stats?.terms || []).filter((t: Term) =>
    t.term.includes(termSearch.toLowerCase()) || t.category.toLowerCase().includes(termSearch.toLowerCase())
  )

  const S = { fontFamily: 'system-ui,-apple-system,sans-serif', minHeight: '100vh', background: '#f8fafc', padding: 24 }
  const card = { background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', padding: 20 }
  const btn = (color = '#1a3a1a', text = '#dffe95') => ({ padding: '8px 18px', background: color, color: text, border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' })

  if (loading) return <div style={{ ...S, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#6b7280' }}>Loading indexer stats…</div>
        ...
        {tab === 'overview' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div style={card}>
              ...
              {runLog.length > 0 && (
                <div style={{ background: '#0f172a', borderRadius: 8, padding: 12, maxHeight: 200, overflowY: 'auto' }}>
                  {runLog.map((l, i) => (
                    <div key={i} style={{ fontSize: 12, color: l.startsWith('❌') ? '#f87171' : l.startsWith('✅') ? '#4ade80' : '#94a3b8', fontFamily: 'monospace', marginBottom: 2 }}>{l}</div>
                  ))}
                </div>
              )}
            </div>
