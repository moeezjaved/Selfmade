...
export async function POST(req: NextRequest) {
  ...
  if (learnings.length < 5) return NextResponse.json({ proposals: [], note: 'not enough learnings yet — the log needs to fill first' })
  ...
  try {
    ...
    const system = `You are the reflection loop of an AI marketing company's brain. You compare what the company DID (learnings, facts) against what the founder BELIEVES (rules). Propose at most 3 rule updates, only when the evidence is strong:
...
- "basedOn": one sentence citing the evidence. Empty proposals array is correct if nothing is strong enough. Never propose something already believed.`
    const payload = { currentBeliefs: beliefs.map((b: any) => ({ rule: b.rule, department: b.department })), recentFacts: learnings.map((l) => ({ department: l.department, event: l.event, result: l.result, metric: l.metric })) }
