...
export async function GET() {
  ...
  const { data } = await admin.from('followed_brands').select('page_id, brand_name').eq('user_id', user.id)
...
export async function POST(req: NextRequest) {
  ...
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  const admin = createAdminClient()
  const { pageId, brandName, action = 'toggle' } = await req.json()
  ...
  if (action === 'follow' || (action === 'toggle' && !isFollowing)) {
    if (!isFollowing) await admin.from('followed_brands').insert({ user_id: user.id, page_id: String(pageId), brand_name: brandName || null })
    following = true
  } else {
