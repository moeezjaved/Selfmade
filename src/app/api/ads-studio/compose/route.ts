...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const prompt = [
      ...
      hasProduct ? `PRODUCT — reference image 2 is the ${brandName} product. Render it EXACTLY as in the photo: same shape, materials, colours, proportions and on-label text — never a different product. Size it at a natural, believable real-world scale (a small handheld device stays small). The subject should be HOLDING or actively USING the product — in their hand or mid-use — in correct proportion to their fingers and face. Make the interaction look real and relaxed (a natural grip, a genuine moment of use), never stiff, awkward or oversized. Unless the founder's art direction says otherwise, the person is engaging with the product, not just posing beside it.` : '',
