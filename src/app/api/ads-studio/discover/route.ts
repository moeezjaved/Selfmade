...
/** The user's spied competitors' IMAGE ads — read from the SAME cached deep pull the competitors feed
 * builds (no live scrape here, so Discover stays instant). Interleaved across brands, deduped by image. */
async function spiedCompetitorImages(admin: any, domain: string): Promise<DiscoverAd[]> {
  try {
    ...
    if (!brandId) return []
    const studioCache = await readAdsStudio(admin, brandId)
    const cached = readSection<{ byPage: Record<string, any>; ranAt: number }>(studioCache, 'spiedAds', domain || 'spied', 1000 * 60 * 60 * 24 * 14)
    const byPage = cached?.byPage || {}
    const perBrand: DiscoverAd[][] = Object.values(byPage).map((v: any) =>
      (Array.isArray(v?.ads) ? v.ads : [])
        .filter((a: any) => a?.thumb && a.format !== 'video')
        .map((a: any) => ({ id: a.id, brand: v?.name || 'Competitor', thumb: a.thumb as string, copy: a.copy || '', format: 'image' as const })))
    // Interleave brands (round-robin) so one competitor doesn't hog the front of the feed, dedup by image.
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // 1. The user's own spied competitors' image ads (from the cached deep pull) — these lead the feed.
    const spied = await spiedCompetitorImages(admin, domain)
