/**
 * GET /api/ads-studio/discover — the workspace "Discover" feed. Puts the LOGGED-IN user's own spied
 * competitors' image ads FIRST (their FÜM etc — live, deduped by creative), then fills the rest with
 * trending images from our crawl library. Each card is "Create Similar" → tags into the Mello chat.
 */
...
// Never hotlink fbcdn — proxy live media through our R2-backed cache.
const mediaUrl = (u?: string | null) => (u ? `/api/ads-studio/media?u=${encodeURIComponent(u)}` : null)
// Signature of the actual image behind a thumb, so we dedup by CREATIVE (Meta repeats the same image under
// many ad ids). Proxied live thumb → inner fbcdn image path; corpus thumb → stable R2 url.
...
/** The user's spied competitors' IMAGE ads (live, images only), newest-brand first. Empty when logged out
 * or no spied brands — the feed then falls back to the global trending images below. */
async function spiedCompetitorImages(admin: any): Promise<DiscoverAd[]> {
  try {
    ...
    const perBrand = await Promise.all(pageIds.map(async (pid) => {
      const live = await fetchLiveAdsByPage(pid, 40).catch(() => [])
  ...
  try {
    ...
    if (rows.length) {
      ...
      for (const a of rows) {
        ...
        if (!thumb || seenBrand.has(brand)) continue   // one per brand, and none already led by a spied card
    ...
    // Spied competitors first, then trending. Keep ALL spied cards; global fills up to the requested count.
    const ads = [...spied, ...globalAds.slice(0, Math.max(0, limit - spied.length))]
