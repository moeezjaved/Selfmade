...
async function handle(req: NextRequest) {
  ...
  const { productImageB64, productImages, productMimeType, newHeadline, brandName, colors, brandId, aspectRatio, angle, artDirection, brief } = body || {}
  ...
  try {
    ...
    const prompt = buildStudioPrompt({
      ...
      brief: typeof brief === 'string' && brief.trim() ? brief.trim().slice(0, 500) : undefined,
      isService,
    })
    ...
    if (best && !isService && products.length && (productMatch === false || sceneHasPeople) && !outOfTime()) {
      const refinePrompt = [
        ...
        `Edit image 1 so EVERY product a person is holding, using or displaying becomes the EXACT product from the reference photo(s): identical silhouette, materials, colour, label/branding and proportions. Keep the existing hand grip, angle and lighting so it looks natural, BUT render the product at its TRUE real-world size — a slim handheld device stays slim and naturally small in the hand, in correct proportion to the fingers and face; never oversized, thick, or dominating the frame. If the current product looks too big or bulky, shrink it to a believable real-world scale.${products.length > 1 ? ' If several different products are shown, match each to the reference it most resembles.' : ''}`,
