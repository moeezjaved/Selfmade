...
export async function POST(req: NextRequest) {
  ...
  if (rErr) {
    ...
    try { const { logError } = await import('@/lib/admin/logError'); void logError({ user_id: user.id, user_email: user.email || null, error_message: insufficient ? `Insufficient credits — video render` : `Credit reserve failed — video render`, page_url: '/studio', extra: { kind: insufficient ? 'insufficient_credits' : 'render_failed', stage: 'video-approve' } }) } catch { /* never block */ }
