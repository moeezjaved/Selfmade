...
export default function AdminWishlist() {
  ...
  return (
    <div style={{ padding: '28px 26px 80px', maxWidth: 820, fontFamily: "'Inter', -apple-system, sans-serif", color: '#161c17' }}>
      ...
      <div style={{ fontSize: 13, color: '#68756b', marginTop: 4 }}>What users asked to see more of in their Morning Brief — newest first. Your build backlog, in their words.</div>

      {items === null && <div style={{ marginTop: 24, color: '#68756b', fontSize: 14 }}>Loading…</div>}
      {items && items.length === 0 && <div style={{ marginTop: 24, color: '#68756b', fontSize: 14 }}>No requests yet. They’ll appear here as users tap “What else do you want to see in your brief?”.</div>}

      {items && items.length > 0 && (
        <div style={{ marginTop: 22, display: 'grid', gap: 10 }}>
          {items.map(it => (
            <div key={it.id} style={{ border: '1px solid #e7ece7', borderRadius: 12, background: '#fff', padding: '14px 16px' }}>
