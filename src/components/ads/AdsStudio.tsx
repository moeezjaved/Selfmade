...
const AD_FORMULAS: AdFormula[] = [
  { key: 'outcome', title: 'Sell the outcome', example: '“Olive oil that makes dinner taste like Italy.”',
    prompt: 'Feature my product prominently. Make an ad whose HEADLINE sells the outcome — name the felt result my customer wants (in the spirit of "olive oil that makes dinner taste like Italy"), grounded in what my product genuinely does. This changes the COPY only: my real product stays the clear physical hero of the image — large, prominent and sharp in the foreground (like the Graza olive-oil bottle sitting on the table), rendered EXACTLY like its reference photo, never shrunk into a small background prop and never morphed into another object like a lamp. Warm, aspirational, editorial styling.' },
  { key: 'scene', title: 'Product is the scene', example: 'Make the product massive — the whole world.',
    prompt: 'Make an ad where my PRODUCT is the entire scene — render my actual product large and hero, and build the whole visual world around the feeling it creates (in the spirit of a giant can pouring out into a dreamy world people relax in). The product is the prominent centrepiece and must be rendered EXACTLY like its reference photo — never morphed into another object. Bold, surreal, product-forward, with one short evocative line of copy.' },
  { key: 'usecase', title: 'Show the use case', example: '“Cracked one before my long run — didn’t cramp once.”',
    prompt: 'Make an ad showing a specific USE CASE — a real moment with a testimonial-style line about exactly when and why someone reaches for my product (in the spirit of "cracked one before my long run, didn\'t even cramp once"). My actual product is clearly visible, held or in active use, rendered EXACTLY like its reference photo at its true handheld size — a slim device stays slim and natural in the hand, in correct proportion to the fingers and face, never oversized or thick. Authentic UGC feel with a handwritten highlight; make the when and the why instantly obvious.' },
  { key: 'replace', title: 'Replace the bad habit', example: '“Your desk drawer is not a meal plan.”',
    prompt: 'Make an ad that calls out the bad habit my product replaces — name the old behaviour bluntly (in the spirit of "your desk drawer is not a meal plan") — then present my actual product as the obvious better solution: prominent and sharp in the foreground, rendered EXACTLY like its reference photo (never shrunk into a small prop or morphed into another object). Punchy, contrarian, big confident type.' },
  { key: 'problem', title: 'Lead with the problem', example: '“Still bloated. Still backed up.”',
    prompt: 'Make an ad that leads with the uncomfortable, specific problem my customer has — say it plainly (in the spirit of "still bloated, still backed up") so they instantly know the ad is for them — then make my actual product the relief: shown prominently and sharply in the foreground as the fix, rendered EXACTLY like its reference photo (never shrunk or morphed). Big blunt headline, a touch of playful visual.' },
  { key: 'metaphor', title: 'Turn it into a metaphor', example: 'One striking image for the benefit.',
    prompt: 'Make an ad that turns my product\'s #1 benefit into ONE striking visual metaphor the eye understands in a second (in the spirit of a cheetah for speed, or clear water for hydration). Make the metaphor the hero, and place my actual product prominently within it (featured or held in the foreground), rendered EXACTLY like its reference photo (never shrunk into a small prop or morphed into another object), with one bold benefit headline.' },
]
