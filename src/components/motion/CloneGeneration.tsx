...
const BURST = [
  { s: 7, c: '#ff5a2c', tx: '2px', ty: '-30px' }, { s: 5, c: '#c7f06e', tx: '24px', ty: '-18px' },
  { s: 7, c: '#ff5a2c', tx: '31px', ty: '6px' }, { s: 5, c: '#fff', tx: '20px', ty: '26px' },
  { s: 7, c: '#ff5a2c', tx: '-4px', ty: '32px' }, { s: 5, c: '#c7f06e', tx: '-24px', ty: '22px' },
  { s: 7, c: '#ff5a2c', tx: '-32px', ty: '2px' }, { s: 5, c: '#fff', tx: '-22px', ty: '-20px' },
]
    ...
    <div style={{ padding: '30px 22px 8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="sm-anim" style={{ position: 'relative', width: 280, maxWidth: '74vw', borderRadius: 16, overflow: 'visible', animation: 'tf-breathe 8.8s linear infinite' }}>
        ...
        <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', background: '#0d120e', aspectRatio: '4 / 5', boxShadow: '0 0 0 1px rgba(14,27,18,.07)' }}>
          ...
          <div className="sm-anim" style={{ position: 'absolute', left: 0, right: 0, height: 2, background: 'linear-gradient(90deg, rgba(255,90,44,0), #ff5a2c 30%, #ff5a2c 70%, rgba(255,90,44,0))', boxShadow: '0 0 14px 2px rgba(255,90,44,.55)', animation: 'tf-scan 8.8s linear infinite' }} />
          {/* act 3 — the headline band (top third, where the copy lives) */}
          <div className="sm-anim" style={{ position: 'absolute', top: '7%', left: '6%', right: '6%', height: '16%', borderRadius: 10, background: 'rgba(255,90,44,.14)', border: '1px solid rgba(255,90,44,.5)', overflow: 'hidden', animation: 'tf-band 8.8s linear infinite' }}>
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ height: 3, background: '#e7e9e4', position: 'relative', overflow: 'hidden', borderRadius: 3 }}>
        <div className="sm-anim" style={{ position: 'absolute', top: 0, height: '100%', width: '42%', borderRadius: 3, background: 'linear-gradient(90deg, rgba(255,90,44,0) 0%, #ff5a2c 50%, rgba(255,90,44,0) 100%)', animation: 'sm-indet 1.5s cubic-bezier(.65,0,.35,1) infinite' }} />
      </div>
      ...
      <div style={{ padding: '34px 22px 8px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        <div className="sm-anim sm-glow" style={{ position: 'absolute', width: 260, height: 300, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,90,44,.9) 0%, rgba(255,90,44,0) 70%)', filter: 'blur(38px)', animation: 'sm-glow 4.5s linear infinite', zIndex: 0 }} />
        <div className="sm-anim" style={{ position: 'relative', zIndex: 1, animation: 'sm-dissolve 4.5s linear infinite' }}>
          <div className="sm-anim" style={{ width: 300, maxWidth: '78vw', aspectRatio: '300 / 380', background: '#fff', borderRadius: 16, boxShadow: '0 18px 40px -14px rgba(14,27,18,.28), 0 0 0 1px rgba(14,27,18,.05)', overflow: 'hidden', display: 'flex', flexDirection: 'column', animation: 'sm-lift 4.5s linear infinite' }}>
            <div style={{ height: '60%', position: 'relative', overflow: 'hidden', background: 'repeating-linear-gradient(135deg,#eef0ec 0 12px,#e7e9e4 12px 24px)' }}>
              <div className="sm-anim" style={{ position: 'absolute', inset: 0, background: 'linear-gradient(150deg,#ff5a2c 0%,#ff7a4c 45%,#141d15 115%)', animation: 'sm-imgfill 4.5s linear infinite' }} />
            ...
            <div style={{ flex: 1, padding: '20px 18px 18px', display: 'flex', flexDirection: 'column' }}>
              <div className="sm-anim" style={{ height: 13, width: '88%', borderRadius: 7, background: '#141d15', transformOrigin: 'left', animation: 'sm-bar1 4.5s linear infinite' }} />
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <div style={{ position: 'relative' }}>
                  <div className="sm-anim" style={{ background: '#ff5a2c', color: '#0e1b12', fontSize: 13, fontWeight: 700, padding: '10px 20px', borderRadius: 999, boxShadow: '0 6px 16px -4px rgba(255,90,44,.7)', transformOrigin: 'center', animation: 'sm-cta 4.5s linear infinite' }}>Shop now</div>
                  {BURST.map((b, i) => (
