 ...
 * (dark sidebar checklist fills, big visual per step) → a gated report (score gauge + sections, each with
 * "Agent can fix") → the offer. Real data from /api/audit/run. Mobile responsive. Preview/branch only.
 */
...
type Section = { key: string; name: string; sub: string; score: number; findings: Finding[] }
type AiRead = { engine: string; mentioned: boolean; question: string; answer: string }
type Result = {
  domain: string; siteName: string; category: string; score: number; grade: string
  websiteScore: number; visibilityScore: number; sections: Section[]
  ai: { question: string; reads: AiRead[] }; revenueLostPerYear: number; currency: string; problemCount: number
}

const STEPS = [
  { key: 'health', label: 'Website health' },
  { key: 'speed', label: 'Website speed' },
  { key: 'spam', label: 'Spam-update check' },
  { key: 'catalog', label: 'Your catalog' },
  { key: 'google', label: 'Google visibility' },
  { key: 'ai', label: 'AI visibility' },
  { key: 'revenue', label: 'Revenue you’re losing' },
]
...
export default function AuditTheater() {
  ...
  const run = useCallback(async () => {
    ...
    setError(null); setPhase('running'); setStep(0)
    // Reveal steps on a timer while the scan runs (~3.2s each → ~22s theater).
    timer.current = setInterval(() => setStep((s) => Math.min(STEPS.length - 1, s + 1)), 3200)
    try {
      const r = await fetch('/api/audit/run', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ domain: d }) })
      const j = await r.json()
      // let the theater play out a beat even if the API is fast
      const settle = () => { clearInterval(timer.current); if (r.ok) { setResult(j); setStep(STEPS.length - 1); setTimeout(() => setPhase('report'), 700) } else { setError(j.error || 'Scan failed.'); setPhase('idle') } }
      setTimeout(settle, Math.max(0, 6000 - 0))
  ...
  /* ── SIDEBAR (shared by running + report) ─────────────────────────────────────────────────── */
  const Sidebar = () => (
    <aside style={{ background: DARK, color: '#fff', padding: isMobile ? 20 : 26, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
      {phase === 'report' && result ? (
        ...
        <>
          <div style={{ fontSize: isMobile ? 18 : 21, fontWeight: 800, letterSpacing: '-.01em' }}>Scanning your website</div>
          ...
          <div style={{ marginTop: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {STEPS.map((s, i) => (
              <div key={s.key} style={{ display: 'flex', alignItems: 'center', gap: 11, opacity: i <= step ? 1 : 0.4 }}>
                ...
                <span style={{ fontSize: 14.5, fontWeight: i === step ? 800 : 600 }}>{s.label}</span>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 'auto', paddingTop: 24 }}>
            <div style={{ height: 6, borderRadius: 100, background: 'rgba(255,255,255,.12)', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${Math.round(((step + 1) / STEPS.length) * 100)}%`, background: LIME, transition: 'width .5s' }} />
            </div>
  ...
  /* ── RUNNING + REPORT share the two-column shell ─────────────────────────────────────────────── */
  return (
    <div style={{ minHeight: '100dvh', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0,300px) minmax(0,1fr)', background: PAPER, fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <main style={{ padding: isMobile ? '22px 18px 60px' : '40px 48px 80px', minWidth: 0, background: PAPER }}>
        {phase === 'running' ? (
          <RunningStage step={step} isMobile={isMobile} />
        ) : result ? (
          <Report result={result} open={open} setOpen={setOpen} isMobile={isMobile} />
        ) : null}
      </main>
    </div>
  )
  ...
  return (
    <>
      ...
      {result.sections.map((sec, si) => (
        <section key={sec.key} style={{ marginBottom: 34 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12, borderBottom: `1px solid ${LINE}`, paddingBottom: 10, marginBottom: 14 }}>
            <div>
              <div style={{ fontSize: isMobile ? 18 : 21, fontWeight: 800, color: INK }}><span style={{ color: SUB, fontWeight: 600, marginRight: 8 }}>{si + 1}.</span>{sec.name}</div>
              <div style={{ fontSize: 13.5, color: SUB, marginTop: 2 }}>{sec.sub}</div>
            </div>
          ...
          {sec.findings.length === 0 ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, border: `1px solid ${LINE}`, borderRadius: 14, padding: 16, background: '#fff' }}>
              <span style={{ color: GOOD, fontSize: 18 }}>✓</span><span style={{ fontSize: 14.5, fontWeight: 600, color: INK }}>Looks good — nothing to fix here.</span>
            </div>
          ) : sec.findings.map((f) => (
            <div key={f.id} style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#fff', marginBottom: 10, overflow: 'hidden' }}>
              <button onClick={() => f.sample?.length && setOpen((o) => ({ ...o, [f.id]: !o[f.id] }))} style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', padding: 16, cursor: f.sample?.length ? 'pointer' : 'default', fontFamily: 'inherit', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                ...
                <span style={{ flex: 1, minWidth: 0 }}>
                  <span style={{ display: 'block', fontSize: 15, fontWeight: 800, color: INK }}>{f.title}</span>
                  <span style={{ display: 'block', fontSize: 13.5, color: SUB, marginTop: 2 }}>{f.detail}</span>
                </span>
              ...
              {open[f.id] && f.sample?.length ? (
                <div style={{ borderTop: `1px solid ${LINE}`, padding: '10px 16px', background: PAPER }}>
                  {f.sample.slice(0, 6).map((s, i) => <div key={i} style={{ fontSize: 12.5, color: '#4a544c', fontFamily: 'monospace', padding: '5px 0', borderTop: i ? `1px solid ${LINE}` : 'none' }}>{s}</div>)}
                </div>
              ) : null}
            </div>
          ))}
          {/* AI answer preview inside the AI section */}
          {sec.key === 'ai' && result.ai.reads.length > 0 && (
            <div style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#fff', padding: 16, marginTop: 4 }}>
              <div style={{ fontSize: 12.5, color: SUB, marginBottom: 8 }}>We asked: <b style={{ color: INK }}>“{result.ai.question}”</b></div>
              <div style={{ fontSize: 13.5, color: '#3a453c', lineHeight: 1.55, maxHeight: 150, overflow: 'hidden', position: 'relative' }}>
                {result.ai.reads[0].answer}
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 50, background: 'linear-gradient(transparent, #fff)' }} />
              </div>
              <div style={{ fontSize: 12.5, color: RED, fontWeight: 700, marginTop: 8 }}>Your store isn’t in the answer.</div>
            </div>
          )}
        </section>
      ))}
      ...
      <section style={{ marginBottom: 20 }}>
        ...
        <div style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#fff', padding: 16, fontSize: 14, color: SUB, lineHeight: 1.6 }}>
          A conservative estimate from the {result.problemCount} fixable problems above and the AI assistants that skip you. Every one is something our agent fixes for you.
        </div>
      </section>
    </>
  )
...
function Gauge({ score, grade }: { score: number; grade: string }) {
  ...
  const col = score >= 60 ? GOOD : score >= 40 ? '#e0a92b' : RED
  return (
    <div style={{ textAlign: 'center', marginBottom: 18 }}>
      <div style={{ position: 'relative', width: 140, height: 140, margin: '0 auto' }}>
        <svg width="140" height="140" style={{ transform: 'rotate(-90deg)' }}>
          <circle cx="70" cy="70" r={r} fill="none" stroke="rgba(255,255,255,.1)" strokeWidth="10" />
          <circle cx="70" cy="70" r={r} fill="none" stroke={col} strokeWidth="10" strokeLinecap="round" strokeDasharray={C} strokeDashoffset={off} />
        </svg>
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ fontSize: 34, fontWeight: 800, color: '#fff', lineHeight: 1 }}>{score}</div>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,.5)' }}>of 100</div>
        </div>
      </div>
...
function SubScore({ label, value }: { label: string; value: number }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, border: '1px solid rgba(255,255,255,.08)', borderRadius: 12, padding: '12px 14px', marginBottom: 10, background: 'rgba(255,255,255,.03)' }}>
      <div style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>{label}</div>
      <div style={{ fontSize: 13, color: value >= 60 ? GOOD : value >= 40 ? '#e0a92b' : RED, fontWeight: 800 }}>{value} of 100</div>
    </div>
  )
}

/* ── Offer (the Grand Slam) ───────────────────────────────────────────────────────────────────── */
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0,1fr) 380px', gap: 28, maxWidth: 1080 }}>
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', gap: 12 }}>
            {[['$5,000/mo of work', 'SEO, content, catalog, competitor intel — a whole team'], ['Fixes in 30 min', 'Meta, alt text, schema — you approve each'], ['Real revenue', 'We bank the organic revenue against every fix'], ['First-Win Guarantee', '30 days or your money back — you keep the work']].map(([t, d], i) => (
              <div key={i} style={{ border: `1px solid ${LINE}`, borderRadius: 14, background: '#fff', padding: 14 }}>
                <div style={{ color: GOOD, fontSize: 18, marginBottom: 6 }}>✓</div>
                <div style={{ fontSize: 14.5, fontWeight: 800, color: INK }}>{t}</div>
                <div style={{ fontSize: 12.5, color: SUB, marginTop: 3, lineHeight: 1.4 }}>{d}</div>
              </div>
            ))}
          </div>
        </div>
        <div style={{ background: DARK, borderRadius: 20, padding: 24, color: '#fff', alignSelf: 'start' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
            <div style={{ fontSize: 19, fontWeight: 800 }}>Growth</div>
            <div style={{ fontSize: 26, fontWeight: 800 }}>$149<span style={{ fontSize: 13, color: 'rgba(255,255,255,.5)', fontWeight: 500 }}>/mo</span></div>
          </div>
          ...
          <div style={{ margin: '18px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {['Every SEO problem found & fixed for you', 'Blog + programmatic pages published monthly', 'Get cited in ChatGPT, Gemini & Perplexity', 'Competitor intel — take their traffic', 'Works with Shopify, WordPress & more'].map((t, i) => (
              <div key={i} style={{ display: 'flex', gap: 9, fontSize: 13.5, color: 'rgba(255,255,255,.9)' }}><span style={{ color: GOOD }}>✓</span>{t}</div>
            ))}
          </div>
